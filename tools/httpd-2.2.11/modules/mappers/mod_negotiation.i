...
int ap_scan_script_header_err_core(request_rec *r, char *buffer,
           ...
           void *getsfunc_data);
# 40 "mod_negotiation.c" 2
# 48 "mod_negotiation.c"
typedef struct {
    int forcelangpriority;
    apr_array_header_t *language_priority;
} neg_dir_config;
# 67 "mod_negotiation.c"
...
{
    return ((((void **)(s->module_config))[(&negotiation_module)->module_index])
                                                      != ((void *)0));
}
...
static void set_mime_fields(var_rec *var, accept_rec *mime_info)
{
    ...
    var->is_pseudo_html = (!strcmp(var->mime_type, "text/html")
                           || !strcmp(var->mime_type, "text/x-server-parsed-html")
                           || !strcmp(var->mime_type, "text/x-server-parsed-html3"));
}
...
# 427 "mod_negotiation.c"
    ...
    else if (!strcmp(result->name, "text/x-server-parsed-html")) {
        result->level = 2.0f;
    }
    else if (!strcmp(result->name, "text/x-server-parsed-html3")) {
        result->level = 3.0f;
    }
...
static char *lcase_header_name_return_body(char *header, request_rec *r)
{
    ...
    if (!*cp) {
        ap_log_rerror("mod_negotiation.c",919, 3, 0, r,
    ...
    if (!*cp) {
        ap_log_rerror("mod_negotiation.c",930, 3, 0, r,
                         ...
                         request_rec *rr)
{
    ...
    if ((status = apr_file_open(map, rr->filename, 0x00001 | 0x00080,
                0x0FFF, neg->pool)) != 0) {
        ap_log_rerror("mod_negotiation.c",958, 3, status, r,
        ...
        if (hstate == header_seen) {
            ...
            if (!strncmp(buffer, "uri:", 4)) {
                ...
                if (apr_strtoff(&number, body, &errp, 10)
                    || *errp || number < 0) {
                    ap_log_rerror("mod_negotiation.c",1001, 3, 0, r,
                ...
                if ((mime_info.body = get_body(buffer, &len, tag, *map)) < 0) {
                    ap_log_rerror("mod_negotiation.c",1036, 3, 0, r,
...
static int read_types_multi(negotiation_state *neg)
{
    ...
    if ((status = apr_dir_open(&dirp, neg->dir_name,
                               neg->pool)) != 0) {
        ap_log_rerror("mod_negotiation.c",1121, 3, status, r,
...
# 1188 "mod_negotiation.c"
        ...
        if (((sub_req->content_type) &&
             !strcmp(sub_req->content_type, "application/x-type-map")) ||
            ((sub_req->handler) &&
    ...
    if (anymatch && !neg->avail_vars->nelts) {
        ap_log_rerror("mod_negotiation.c",1293, 3, 0, r,
    ...
    if (accept_type[0] == '*') {
        ...
        return 1;
             ...
             || (!strcmp(accept_type, "text/html")
                 && (!strcmp(avail_type, "text/x-server-parsed-html")
                     || !strcmp(avail_type, "text/x-server-parsed-html3")))) {
...
static void set_charset_quality(negotiation_state *neg, var_rec *variant)
{
    ...
    if (charset == ((void *)0) || !*charset) {
        ...
        if (!(!strncmp(variant->mime_type, "text/", 5)
              || !strcmp(variant->mime_type, "text/x-server-parsed-html")
              || !strcmp(variant->mime_type, "text/x-server-parsed-html3")
              ))
            return;
...
# 2425 "mod_negotiation.c"
    *pbest = best;
    return algorithm_result;
}
# 2446 "mod_negotiation.c"
                                 ...
                                 var_rec *variant)
{
    ...
    if (!variant->sub_req) {
        ...
        sub_req = ap_sub_req_lookup_file(variant->file_name, r, ((void *)0));
...
# 2751 "mod_negotiation.c"
    if (neg->is_transparent &&
        apr_table_get(sub_req->err_headers_out, "TCN")) {
        return 506;
    }
# 2779 "mod_negotiation.c"
    if (sub_req->handler && strcmp(sub_req->handler, "type-map") == 0) {
        return 506;
    }
# 2795 "mod_negotiation.c"
...
# 2902 "mod_negotiation.c"
            return 300;
        }

        if (!*bestp) {
            ap_log_rerror("mod_negotiation.c",2906, 3, 0, r,
...
static int handle_map_file(request_rec *r)
{
    ...
    if(strcmp(r->handler,"application/x-type-map") && strcmp(r->handler,"type-map"))
        return -1;
...
static int handle_multi(request_rec *r)
{
    ...
    if (!(sub_req = best->sub_req)) {
        ...
        sub_req = ap_sub_req_lookup_file(best->file_name, r, ((void *)0));
        if (sub_req->status != 200) {
            res = sub_req->status;
            ap_destroy_sub_req(sub_req);
            goto return_from_multi;
...
module negotiation_module =
{
    20051115, 21, -1, "mod_negotiation.c", ((void *)0), ((void *)0), 0x41503232UL, ((void *)0),
