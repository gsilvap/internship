...
{
    alias_server_conf *a =
    (alias_server_conf *) memset(apr_palloc(p, sizeof(alias_server_conf)), 0, sizeof(alias_server_conf));
...
{
    alias_dir_conf *a =
    (alias_dir_conf *) memset(apr_palloc(p, sizeof(alias_dir_conf)), 0, sizeof(alias_dir_conf));
                                      ...
                                      int use_regex)
{
    ...
    alias_server_conf *conf = (((void **)(s->module_config))[(&alias_module)->module_index])
                                                                 ;
    ...
    if (!use_regex) {
        for (i = 0; i < conf->aliases->nelts - 1; ++i) {
            ...
            if ( (!p->regexp && alias_matches(f, p->fake) > 0)
                || (p->regexp && !ap_regexec(p->regexp, f, 0, ((void *)0), 0))) {
                ap_log_error("mod_alias.c",140, 4, 0, cmd->server,
                                         ...
                                         const char *arg3, int use_regex)
{
    alias_entry *new;
    server_rec *s = cmd->server;
    alias_server_conf *serverconf = (((void **)(s->module_config))[(&alias_module)->module_index])
                                                                       ;
...
static const char *add_redirect2(cmd_parms *cmd, void *dirconf,
                                 const char *arg1, const char *arg2)
{
    return add_redirect_internal(cmd, dirconf, arg1, arg2, ((void *)0), 0);
}
...
{
    { "Alias", { .take2=add_alias }, ((void *)0), 128, TAKE2, "a fakename and a realname" }
                                              ,
    { "ScriptAlias", { .take2=add_alias }, "cgi-script", 128, TAKE2, "a fakename and a realname" }
                                              ,
    { "Redirect", { .take3=add_redirect }, (void *) 302, 4, TAKE23, "an optional status, then document to be redirected and " "destination URL" }


                                     ,
    { "AliasMatch", { .take2=add_alias_regex }, ((void *)0), 128, TAKE2, "a regular expression and a filename" }
                                                        ,
    { "ScriptAliasMatch", { .take2=add_alias_regex }, "cgi-script", 128, TAKE2, "a regular expression and a filename" }
                                                        ,
    { "RedirectMatch", { .take3=add_redirect_regex }, (void *) 302, 4, TAKE23, "an optional status, then a regular expression and " "destination URL" }


                                     ,
    { "RedirectTemp", { .take2=add_redirect2 }, (void *) 302, 4, TAKE2, "a document to be redirected, then the destination URL" }

                                                                          ,
    { "RedirectPermanent", { .take2=add_redirect2 }, (void *) 301, 4, TAKE2, "a document to be redirected, then the destination URL" }
                            ...
                            int doesc, int *status)
{
    ...
    for (i = 0; i < aliases->nelts; ++i) {
        ...
        if (p->regexp) {
            if (!ap_regexec(p->regexp, r->uri, 10, regm, 0)) {
                if (p->real) {
                    ...
                    if (found && doesc) {
                        ...
                        found = apr_uri_unparse(r->pool, &uri,
                                                (1U<<5));
                        found = ap_os_escape_path(r->pool,found,1);
                        if (uri.query) {
...
static int translate_alias_redir(request_rec *r)
{
    ...
    alias_server_conf *serverconf = (((void **)(sconf))[(&alias_module)->module_index]);
    char *ret;
    int status;
    ...
    if ((ret = try_alias_list(r, serverconf->redirects, 1, &status)) != ((void *)0)) {
        if ((((status) >= 300)&&((status) < 400))) {
    ...
    if ((ret = try_alias_list(r, serverconf->aliases, 0, &status)) != ((void *)0)) {
        r->filename = ret;
        return 0;
    }
...
static int fixup_redir(request_rec *r)
{
    ...
    alias_dir_conf *dirconf =
    (alias_dir_conf *) (((void **)(dconf))[(&alias_module)->module_index]);
    char *ret;
    int status;



    if ((ret = try_alias_list(r, dirconf->redirects, 1, &status)) != ((void *)0)) {
        if ((((status) >= 300)&&((status) < 400))) {
            if (ret[0] == '/') {
                char *orig_target = ret;

                ret = ap_construct_url(r->pool, ret, r);
                ap_log_rerror("mod_alias.c",441, 7, 0, r,
            ...
            if (!ap_is_url(ret)) {
                status = 500;
                ap_log_rerror("mod_alias.c",448, 3, 0, r,
...
static void register_hooks(apr_pool_t *p)
{
    static const char * const aszSucc[]={ "mod_userdir.c",
                                          "mod_vhost_alias.c",((void *)0) };

    ap_hook_translate_name(translate_alias_redir,((void *)0),aszSucc,10);
    ap_hook_fixups(fixup_redir,((void *)0),((void *)0),10);
}

module alias_module =
{
    20051115, 21, -1, "mod_alias.c", ((void *)0), ((void *)0), 0x41503232UL, ((void *)0),
