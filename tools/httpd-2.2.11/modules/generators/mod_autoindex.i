...
# 26 "/usr/include/apr-1/apr_network_io.h"
...
# 39 "mod_autoindex.c" 2
# 1 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/http_core.h" 1
# 30 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/http_core.h"
...
# 31 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/http_core.h" 2
...
# 36 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/http_core.h" 2
...
# 37 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/http_core.h" 2
...
# 42 "mod_autoindex.c" 2
# 1 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/http_log.h" 1
# 33 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/http_log.h"
# 1 "/usr/include/apr-1/apr_thread_proc.h" 1
# 34 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/http_log.h" 2
...
int ap_scan_script_header_err_core(request_rec *r, char *buffer,
           ...
           void *getsfunc_data);
# 45 "mod_autoindex.c" 2

# 1 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/modules/http/mod_core.h" 1
# 34 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/modules/http/mod_core.h"
# 1 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/util_filter.h" 1
# 35 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/modules/http/mod_core.h" 2
...
static void emit_preamble(request_rec *r, int xhtml, const char *title)
{
    ...
    if (xhtml) {
        ap_rvputs(r, "<!DOCTYPE html PUBLIC \"-//W3C//" "DTD XHTML 1.0 Transitional//EN\"\n" "\"http://www.w3.org/TR/xhtml1/DTD/" "xhtml1-transitional.dtd\">\n",
                      ...
                      const char *path, const char *data)
{
    ...
    p->type = type;
    p->data = data ? apr_pstrdup(arr->pool, data) : ((void *)0);
    p->apply_path = apr_pstrcat(arr->pool, path, "*", ((void *)0));

    if ((type == &c_by_path) && (!ap_is_matchexp(to))) {
...
static const char *add_alt(cmd_parms *cmd, void *d, const char *alt,
                           const char *to)
{
    if (cmd->info == &c_by_path) {
        if (!strcmp(to, "**DIRECTORY**")) {
            to = "^^DIRECTORY^^";
        }
    }
    if (cmd->info == &c_by_encoding) {
                            ...
                            const char *to)
{
    ...
    desc_entry->full_path = (strchr(to, '/') == ((void *)0)) ? 0 : 1;
    desc_entry->wildcards = (0
...
{
    push_item(((autoindex_config_rec *) d)->ign_list, 0, ext, cmd->path, ((void *)0));
    return ((void *)0);
}

static const char *add_header(cmd_parms *cmd, void *d, const char *name)
{
    push_item(((autoindex_config_rec *) d)->hdr_list, 0, ((void *)0), cmd->path,
              name);
    return ((void *)0);
}
...
{
    push_item(((autoindex_config_rec *) d)->rdme_list, 0, ((void *)0), cmd->path,
              name);
    return ((void *)0);
}
                                     ...
                                     const char *direction, const char *key)
{
    ...
    if (!strcasecmp(direction, "Ascending")) {
        d_cfg->default_direction = 'A';
    }
    else if (!strcasecmp(direction, "Descending")) {
        d_cfg->default_direction = 'D';
    }
    else {
    ...
    if (!strcasecmp(key, "Name")) {
        d_cfg->default_keyid = 'N';
    }
    else if (!strcasecmp(key, "Date")) {
        d_cfg->default_keyid = 'M';
    }
    else if (!strcasecmp(key, "Size")) {
        d_cfg->default_keyid = 'S';
    }
    else if (!strcasecmp(key, "Description")) {
        d_cfg->default_keyid = 'D';
    }
    else {
...
{
    { "AddIcon", { .take2=add_icon }, &c_by_path, 16, ITERATE2, "an icon URL followed by one or more filenames" }
                                                                     ,
    { "AddIconByType", { .take2=add_icon }, &c_by_type, 16, ITERATE2, "an icon URL followed by one or more MIME types" }
                                                                      ,
    { "AddIconByEncoding", { .take2=add_icon }, &c_by_encoding, 16, ITERATE2, "an icon URL followed by one or more content encodings" }
                                                                             ,
    { "AddAlt", { .take2=add_alt }, &c_by_path, 16, ITERATE2, "alternate descriptive text followed by one or more " "filenames" }

                                 ,
    { "AddAltByType", { .take2=add_alt }, &c_by_type, 16, ITERATE2, "alternate descriptive text followed by one or more MIME " "types" }

                             ,
    { "AddAltByEncoding", { .take2=add_alt }, &c_by_encoding, 16, ITERATE2, "alternate descriptive text followed by one or more " "content encodings" }

                                         ,
    { "IndexOptions", { .take_argv=add_opts }, ((void *)0), 16, TAKE_ARGV, "one or more index options [+|-][]" }
                                                          ,
    { "IndexOrderDefault", { .take2=set_default_order }, ((void *)0), 16, TAKE2, "{Ascending,Descending} {Name,Size,Description,Date}" }
                                                                        ,
    { "IndexIgnore", { .take1=add_ignore }, ((void *)0), 16, ITERATE, "one or more file extensions" }
                                                  ,
    { "AddDescription", { .take2=add_desc }, &c_by_path, 16, ITERATE2, "Descriptive text followed by one or more filenames" }
                                                                          ,
    { "HeaderName", { .take1=add_header }, ((void *)0), 16, TAKE1, "a filename" }
                               ,
    { "ReadmeName", { .take1=add_readme }, ((void *)0), 16, TAKE1, "a filename" }
                               ,
    { "FancyIndexing", { .raw_args=ap_set_deprecated }, ((void *)0), (1|2|4|8|16), RAW_ARGS, "The FancyIndexing directive is no longer supported. " "Use IndexOptions FancyIndexing." }

                                                       ,
    { "DefaultIcon", { .take1=ap_set_string_slot }, (void *)((long) (((char *) (&(((autoindex_config_rec*)((void *)0))->default_icon))) - ((char *) ((void *)0)))), 16, TAKE1, "an icon URL" }

                                               ,
    { "IndexStyleSheet", { .take1=ap_set_string_slot }, (void *)((long) (((char *) (&(((autoindex_config_rec*)((void *)0))->style_sheet))) - ((char *) ((void *)0)))), 16, TAKE1, "URL to style sheet" }

                                                      ,
    { "IndexHeadInsert", { .take1=ap_set_string_slot }, (void *)((long) (((char *) (&(((autoindex_config_rec*)((void *)0))->head_insert))) - ((char *) ((void *)0)))), 16, TAKE1, "String to insert in HTML HEAD section" }
...
static void *create_autoindex_config(apr_pool_t *p, char *dummy)
{
    ...
    (autoindex_config_rec *) memset(apr_palloc(p, sizeof(autoindex_config_rec)), 0, sizeof(autoindex_config_rec));
...
static void *merge_autoindex_configs(apr_pool_t *p, void *basev, void *addv)
{
    ...
    new = (autoindex_config_rec *) memset(apr_palloc(p, sizeof(autoindex_config_rec)), 0, sizeof(autoindex_config_rec));
...
static char *find_item(request_rec *r, apr_array_header_t *list, int path_only)
{
    ...
    for (i = 0; i < list->nelts; ++i) {
        ...
        if ((path[0] == '^') || (!ap_strcmp_match(path, p->apply_path))) {
            ...
            else if (p->type == &c_by_path || path[0] == '^') {
...
static char *find_default_item(char *bogus_name, apr_array_header_t *list)
{
    ...
    r.content_type = r.content_encoding = ((void *)0);
...
static char *find_desc(autoindex_config_rec *dcfg, const char *filename_full)
{
    ...
    if ((filename_only = strrchr(filename_full, '/')) == ((void *)0)) {
    ...
    for (i = 0; i < dcfg->desc_list->nelts; ++i) {
        ...
        if (tuple->wildcards) {
            found = (apr_fnmatch(tuple->pattern, filename, 0) == 0);
        }
        else {
            found = (strstr(filename, tuple->pattern) != ((void *)0));
        }
        if (found) {
...
static int ignore_entry(autoindex_config_rec *d, char *path)
{
    ...
    for (i = 0; i < list->nelts; ++i) {
        ...
        if ((ap = strrchr(p->apply_to, '/')) == ((void *)0)) {
            ap = p->apply_to;
        }
        else {
            ap++;
...
# 1062 "mod_autoindex.c"
                ...
                if (apr_file_open(&f, rr->filename, 0x00001,
                                  0x0FFF, r->pool) == 0) {
                    emit_preamble(r, emit_xhtml, title);
                    emit_amble = 0;
                    do_emit_plain(r, f);
...
{
    char titlebuf[8192], *find = "<title>";
    apr_file_t *thefile = ((void *)0);
    int x, y, p;
    apr_size_t n;
    ...
    if ((r->content_type != ((void *)0))
        && (!strcasecmp(ap_field_noparam(r->pool, r->content_type),
                        "text/html")
            || !strcmp(r->content_type, "text/x-server-parsed-html"))
        && !r->content_encoding) {
        if (apr_file_open(&thefile, r->filename, 0x00001,
                          0x0FFF, r->pool) != 0) {
            return ((void *)0);
        }
        n = sizeof(char) * (8192 - 1);
        ...
        for (x = 0, p = 0; titlebuf[x]; x++) {
            if ((tolower(((unsigned char)(titlebuf[x])))) == find[p]) {
                if (!find[++p]) {
                    ...
                    for (y = x; titlebuf[y]; y++) {
                        if ((titlebuf[y] == 13) || (titlebuf[y] == 10)) {
                            if (y == x) {
                                x++;
                            }
                            else {
...
static struct ent *make_parent_entry(apr_int32_t autoindex_opts,
                                     ...
                                     char direction)
{
    struct ent *p = (struct ent *) memset(apr_palloc(r->pool, sizeof(struct ent)), 0, sizeof(struct ent));
    char *testpath;
# 1264 "mod_autoindex.c"
    ...
    p->size = -1;
    p->lm = -1;
    p->key = (toupper(((unsigned char)(keyid))));
    p->ascending = ((toupper(((unsigned char)(direction)))) == 'A');
    p->version_sort = autoindex_opts & (1 << 11);
    if (autoindex_opts & (1 << 13)) {
        if (!(p->icon = find_default_item(testpath, d->icon_list))) {
            p->icon = find_default_item("^^DIRECTORY^^", d->icon_list);
        }
        if (!(p->alt = find_default_item(testpath, d->alt_list))) {
            if (!(p->alt = find_default_item("^^DIRECTORY^^", d->alt_list))) {
                p->alt = "DIR";
            }
        }
                                        ...
                                        const char *pattern)
{
    ...
    if (!(rr = ap_sub_req_lookup_dirent(dirent, r, 0, ((void *)0)))) {
        return (((void *)0));
    }

    if((autoindex_opts & (1 << 18))
        && (rr->status == 401 || rr->status == 403)) {
        show_forbidden = 1;
    }

    if ((rr->finfo.filetype != APR_DIR && rr->finfo.filetype != APR_REG)
        || !(rr->status == 0 || (((rr->status) >= 200)&&((rr->status) < 300))
                              || (((rr->status) >= 300)&&((rr->status) < 400))
                              || show_forbidden == 1)) {
        ap_destroy_sub_req(rr);
        return (((void *)0));
    }

    p = (struct ent *) memset(apr_palloc(r->pool, sizeof(struct ent)), 0, sizeof(struct ent));
    ...
    p->ascending = ((toupper(((unsigned char)(direction)))) == 'A');
    p->version_sort = !!(autoindex_opts & (1 << 11));
    p->ignore_case = !!(autoindex_opts & (1 << 16));

    if (autoindex_opts & ((1 << 13) | (1 << 14))) {
        p->lm = rr->finfo.mtime;
        if (dirent->filetype == APR_DIR) {
            if (autoindex_opts & (1 << 10)) {
                p->isdir = 1;
            }
            ...
            if (!(p->icon = find_item(rr,d->icon_list,1))) {
                p->icon = find_default_item("^^DIRECTORY^^", d->icon_list);
            }
            if (!(p->alt = find_item(rr,d->alt_list,1))) {
                if (!(p->alt = find_default_item("^^DIRECTORY^^", d->alt_list))) {
                    p->alt = "DIR";
                }
            }
                                   ...
                                   apr_int32_t autoindex_opts, int desc_width)
{
    ...
    if (d->desc_adjust == 2) {
        if (autoindex_opts & (1 << 3)) {
            maxsize += 6;
        }
        if (autoindex_opts & (1 << 4)) {
            maxsize += 19;
        }
        if (autoindex_opts & (1 << 5)) {
            maxsize += 7;
        }
    }
                      ...
                      const char *colargs, int nosort)
{
    if (!nosort) {
        ...
        qvalue[7] = ((curkey == column) && (curdirection == 'A'))
                      ? 'D' : 'A';
        qvalue[8] = '\0';
                               ...
                               char direction, const char *colargs)
{
    ...
    char *name = r->uri;
    char *tp;
    int static_columns = !!(autoindex_opts & (1 << 8));
    apr_pool_t *scratch;
    int name_width;
    int desc_width;
    char *name_scratch;
    ...
    apr_pool_create_ex(&scratch, r->pool, ((void *)0), ((void *)0));
    if (name[0] == '\0') {
        name = "/";
    }
    ...
    if (autoindex_opts & (1 << 14)) {
        int cols = 1;
        ap_rputs("<table><tr>", r);
        if (!(autoindex_opts & (1 << 3))) {
            ap_rputs("<th>", r);
            if ((tp = find_default_item("^^BLANKICON^^", d->icon_list))) {
        ...
        ap_rputs("<th>", r);
        emit_link(r, "Name", 'N', keyid, direction,
                  colargs, static_columns);
        if (!(autoindex_opts & (1 << 4))) {
            ap_rputs("</th><th>", r);
            emit_link(r, "Last modified", 'M', keyid, direction,
                      colargs, static_columns);
            ++cols;
        }
        if (!(autoindex_opts & (1 << 5))) {
            ap_rputs("</th><th>", r);
            emit_link(r, "Size", 'S', keyid, direction,
                      colargs, static_columns);
            ++cols;
        }
        if (!(autoindex_opts & (1 << 6))) {
            ap_rputs("</th><th>", r);
            emit_link(r, "Description", 'D', keyid, direction,
                      colargs, static_columns);
            ++cols;
        }
        if (!(autoindex_opts & (1 << 9))) {
        ...
        if (autoindex_opts & (1 << 14)) {
            ...
            if (d->name_adjust == 1) {
                ap_rvputs(r, "<td><a href=\"", anchor, "\">",
                          ap_escape_html(scratch, t2), "</a>", ((void *)0));
            }
            else {
                nwidth = strlen(t2);
            ...
            if (!(autoindex_opts & (1 << 5))) {
                ...
                ap_rvputs(r, "</td><td align=\"right\">",
                          apr_strfsize(ar[x]->size, buf), ((void *)0));
            }
            if (!(autoindex_opts & (1 << 6))) {
                if (ar[x]->desc) {
                    if (d->desc_adjust == 1) {
                           ...
                           autoindex_config_rec *autoindex_conf)
{
    ...
    if ((status = apr_dir_open(&thedir, name, r->pool)) != 0) {
        ap_log_rerror("mod_autoindex.c",2007, 3, status, r,
    ...
    keyid = autoindex_conf->default_keyid
                ? autoindex_conf->default_keyid : 'N';
    direction = autoindex_conf->default_direction
                ? autoindex_conf->default_direction : 'A';
# 2059 "mod_autoindex.c"
    if (autoindex_opts & (1 << 15)) {
        qstring = ((void *)0);
        autoindex_opts |= (1 << 8);
        colargs = "";
    }
        ...
        while (qstring && *qstring) {
            ...
            if ( qstring[0] == 'C' && qstring[1] == '='
                && qstring[2] && strchr("NMSD", qstring[2])
                && ( qstring[3] == '&' || qstring[3] == ';'
                    || !qstring[3])) {
                keyid = qstring[2];
                ...
                qstring += qstring[3] ? 4 : 3;
                     ...
                     && ( qstring[3] == '&' || qstring[3] == ';'
                         || !qstring[3])) {
                if (qstring[2] == '0') {
                    autoindex_opts &= ~((1 << 13) | (1 << 14));
                }
                else if (qstring[2] == '1') {
                    autoindex_opts = (autoindex_opts | (1 << 13))
                        & ~(1 << 14);
                }
                else if (qstring[2] == '2') {
                    autoindex_opts |= (1 << 13) | (1 << 14);
                ...
                if (eos == qstring) {
                    ...
                    if (ap_unescape_url(pstring) != 0) {
                        ...
                        epattern = ap_os_escape_path(r->pool,pstring,1);
                    }
                }
        ...
        colargs = apr_pstrcat(r->pool, fval, vval, ppre, epattern, ((void *)0));
    }
    ...
    emit_head(r, find_item(r,autoindex_conf->hdr_list,0),
              autoindex_opts & (1 << 7),
              autoindex_opts & (1 << 17), title_name);
    ...
    fullpath = apr_palloc(r->pool, 4096);
    dirpathlen = strlen(name);
    memcpy(fullpath, name, dirpathlen);

    do {
        status = apr_dir_read(&dirent, 0x00008170 | 0x02000000, thedir);
        if (((status) == ((20000 + 50000) + 8))) {
            continue;
        }
        else if (status != 0) {
            break;
        ...
        if (dirent.filetype == APR_LNK) {
            ...
            status = apr_stat(&fi, fullpath,
                              dirent.valid & ~(0x02000000), r->pool);
            if (status != 0) {
    ...
    emit_tail(r, find_item(r,autoindex_conf->rdme_list,0),
              autoindex_opts & (1 << 7));
...
static int handle_autoindex(request_rec *r)
{
    ...
    if(strcmp(r->handler,"httpd/unix-directory")) {
        return -1;
    }
    ...
    if (allow_opts & 1) {
        ...
        ap_log_rerror("mod_autoindex.c",2292, 3, 0, r,
...
{
    ap_hook_handler(handle_autoindex,((void *)0),((void *)0),10);
}

module autoindex_module =
{
    20051115, 21, -1, "mod_autoindex.c", ((void *)0), ((void *)0), 0x41503232UL, ((void *)0),
