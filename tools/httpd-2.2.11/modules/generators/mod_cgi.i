...
# 1 "mod_cgi.h" 1
# 29 "mod_cgi.h"
# 1 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/modules/filters/mod_include.h" 1
# 69 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/modules/filters/mod_include.h"
typedef struct {
...
# 63 "mod_cgi.h"
typedef apr_status_t (apr_OFN_ap_cgi_build_command_t) (const char **cmd, const char ***argv, request_rec *r, apr_pool_t *p, cgi_exec_info_t *e_info)
...
{
    cgi_server_conf *c =
    (cgi_server_conf *) memset(apr_palloc(p, sizeof(cgi_server_conf)), 0, sizeof(cgi_server_conf));
...
static const char *set_scriptlog(cmd_parms *cmd, void *dummy, const char *arg)
{
    ...
    cgi_server_conf *conf = (((void **)(s->module_config))[(&cgi_module)->module_index])
                                                             ;
                           ...
                           apr_status_t rv, char *error)
{
    ...
    ap_log_rerror("mod_cgi.c",168, log_flags, rv, r,
    ...
    if (!conf->logname ||
        ...
        (apr_file_open(&f, conf->logname,
                       0x00008|0x00002|0x00004, 0x0FFF,
                       r->pool) != 0)) {
        return ret;
    }
...
static apr_status_t log_script_err(request_rec *r, apr_file_t *script_err)
{
    ...
    while ((rv = apr_file_gets(argsbuffer, 8192,
                               script_err)) == 0) {
        newline = strchr(argsbuffer, '\n');
        if (newline) {
            *newline = '\0';
        }
        ap_log_rerror("mod_cgi.c",209, 3, 0, r,
                      "%s", argsbuffer);
    }
                      ...
                      apr_file_t *script_err)
{
    ...
    if (apr_file_gets(argsbuffer, 8192, script_err) == 0) {
...
static void add_ssi_vars(request_rec *r)
{
    ...
    if (r->path_info && r->path_info[0] != '\0') {
        ...
        pa_req = ap_sub_req_lookup_uri(ap_os_escape_path(r->pool,r->path_info,1),
                                       r, ((void *)0));
        if (pa_req->filename) {
            apr_table_setn(e, "PATH_TRANSLATED",
                           apr_pstrcat(r->pool, pa_req->filename,
                                       pa_req->path_info, ((void *)0)));
        }
                                  ...
                                  cgi_exec_info_t *e_info)
{
    ...
    core_dir_config *conf = (((void **)(r->per_dir_config))[(&core_module)->module_index])
                                                              ;
# 404 "mod_cgi.c"
    ;
...
# 421 "mod_cgi.c"
    if (((rc = apr_procattr_create(&procattr, p)) != 0) ||
        ...
        ((rc = apr_procattr_limit_set(procattr, 0,
                                      conf->limit_cpu)) != 0) ||


        ((rc = apr_procattr_limit_set(procattr, 1,
                                      conf->limit_mem)) != 0) ||


        ((rc = apr_procattr_limit_set(procattr, 2,
                                      conf->limit_nproc)) != 0) ||
        ...
        ((rc = apr_procattr_addrspace_set(procattr,
                                        e_info->addrspace)) != 0) ||
        ((rc = apr_procattr_child_errfn_set(procattr, cgi_child_errfn)) != 0)) {

        ap_log_rerror("mod_cgi.c",450, 3, rc, r,
        ...
        if (rc != 0) {
            ...
            ap_log_rerror("mod_cgi.c",460, 3|((0x07 + 1) * 2), rc, r,
                                    ...
                                    const char **str, apr_size_t *len)
{
    ...
    *len = 8000;
    ...
    if (*len > 0) {
        ...
        h->alloc_len = 8000;
        *str = buf;
        do { apr_bucket *ap__a = (a), *ap__b = (cgi_bucket_dup(data, a->list)); do { (((ap__b)))->link.prev = ((ap__a)); (((ap__b)))->link.next = (((ap__a)))->link.next; ((((ap__a)))->link.next)->link.prev = ((ap__b)); (((ap__a)))->link.next = ((ap__b)); } while (0); ; } while (0);
    }
    else {
        apr_bucket_free(buf);
                                    ...
                                    apr_size_t *len, apr_read_type_e block)
{
    ...
    do {
        ...
        if (((rv) == ((20000 + 50000) + 7))) {
            if (timeout) {
                ap_log_rerror("mod_cgi.c",682, 4, 0, data->r,
            ...
            ap_log_rerror("mod_cgi.c",695, 3, rv, data->r,
...
static int cgi_handler(request_rec *r)
{
    ...
    const char **argv;
    char *dbuf = ((void *)0);
    apr_file_t *script_out = ((void *)0), *script_in = ((void *)0), *script_err = ((void *)0);
    apr_bucket_brigade *bb;
    apr_bucket *b;
    int is_included;
    ...
    if(strcmp(r->handler, "application/x-httpd-cgi") && strcmp(r->handler, "cgi-script"))
        return -1;
    ...
    conf = (((void **)(r->server->module_config))[(&cgi_module)->module_index]);

    if (!(ap_allow_options(r) & 8) && !is_scriptaliased(r))
        return log_scripterror(r, conf, 403, 0,
    ...
    if (r->finfo.filetype == 0)
        return log_scripterror(r, conf, 404, 0,
...
# 792 "mod_cgi.c"
    ...
    if ((rv = cgi_build_command(&command, &argv, r, p, &e_info)) != 0) {
        ap_log_rerror("mod_cgi.c",809, 3, rv, r,
    ...
    if ((rv = run_cgi_child(&script_out, &script_in, &script_err,
                            command, argv, r, p, &e_info)) != 0) {
        ap_log_rerror("mod_cgi.c",818, 3, rv, r,
    ...
    do {
        ...
        if (rv != 0) {
            ap_log_rerror("mod_cgi.c",840, 3, rv, r,
             ...
             bucket = ((bucket))->link.next)
        {
            ...
            rv = apr_file_write_full(script_out, data, len, ((void *)0));
                                ...
                                apr_bucket_brigade *bb, const char *command)
{
    ...
    if ((rv = cgi_build_command(&command, &argv, r, r->pool,
                                &e_info)) != 0) {
        ap_log_rerror("mod_cgi.c",1110, 3, rv, r,
    ...
    if ((rv = run_cgi_child(&script_out, &script_in, &script_err,
                            ...
                            &e_info)) != 0) {
        ap_log_rerror("mod_cgi.c",1120, 3, rv, r,
                      "couldn't spawn child process: %s", r->filename);
        return rv;
    }
                                ...
                                apr_bucket_brigade *bb)
{
    ...
    if (ctx->flags & (1<<3)) {
        ap_log_rerror("mod_cgi.c",1163, 3, 0, r, "exec used but not allowed "
                      "in %s", r->filename);
        do { apr_bucket *ap__b = (apr_bucket_pool_create(apr_pstrdup((ctx)->pool, (ctx)->error_str), strlen((ctx)->error_str), (ctx)->pool, (f)->c->bucket_alloc)); do { ((((ap__b))))->link.next = ((struct apr_bucket *)((char *)(&(((&((bb))->list)))->next) - ((long) (((char *) (&(((struct apr_bucket*)((void *)0))->link))) - ((char *) ((void *)0)))))); ((((ap__b))))->link.prev = (((struct apr_bucket *)((char *)(&(((&((bb))->list)))->next) - ((long) (((char *) (&(((struct apr_bucket*)((void *)0))->link))) - ((char *) ((void *)0)))))))->link.prev; ((((struct apr_bucket *)((char *)(&(((&((bb))->list)))->next) - ((long) (((char *) (&(((struct apr_bucket*)((void *)0))->link))) - ((char *) ((void *)0)))))))->link.prev)->link.next = (((ap__b))); (((struct apr_bucket *)((char *)(&(((&((bb))->list)))->next) - ((long) (((char *) (&(((struct apr_bucket*)((void *)0))->link))) - ((char *) ((void *)0)))))))->link.prev = (((ap__b))); } while (0); ; } while (0);
        return 0;
    }
    ...
    while (1) {
        ...
        if (!strcmp(tag, "cmd")) {
            ...
            if (rv != 0) {
                ap_log_rerror("mod_cgi.c",1183, 3, 0, r, "execution failure "
            ...
            if (rv != 0) {
                ap_log_rerror("mod_cgi.c",1198, 3, 0, r, "invalid CGI ref "
                              "\"%s\" in %s", tag_val, file);
                do { apr_bucket *ap__b = (apr_bucket_pool_create(apr_pstrdup((ctx)->pool, (ctx)->error_str), strlen((ctx)->error_str), (ctx)->pool, (f)->c->bucket_alloc)); do { ((((ap__b))))->link.next = ((struct apr_bucket *)((char *)(&(((&((bb))->list)))->next) - ((long) (((char *) (&(((struct apr_bucket*)((void *)0))->link))) - ((char *) ((void *)0)))))); ((((ap__b))))->link.prev = (((struct apr_bucket *)((char *)(&(((&((bb))->list)))->next) - ((long) (((char *) (&(((struct apr_bucket*)((void *)0))->link))) - ((char *) ((void *)0)))))))->link.prev; ((((struct apr_bucket *)((char *)(&(((&((bb))->list)))->next) - ((long) (((char *) (&(((struct apr_bucket*)((void *)0))->link))) - ((char *) ((void *)0)))))))->link.prev)->link.next = (((ap__b))); (((struct apr_bucket *)((char *)(&(((&((bb))->list)))->next) - ((long) (((char *) (&(((struct apr_bucket*)((void *)0))->link))) - ((char *) ((void *)0)))))))->link.prev = (((ap__b))); } while (0); ; } while (0);
                break;
            }
            ...
            ap_log_rerror("mod_cgi.c",1205, 3, 0, r, "unknown parameter "
...
static void register_hooks(apr_pool_t *p)
{
    ...
    ap_hook_handler(cgi_handler, ((void *)0), ((void *)0), 10);
    ap_hook_post_config(cgi_post_config, aszPre, ((void *)0), (-10));
}

module cgi_module =
{
    20051115, 21, -1, "mod_cgi.c", ((void *)0), ((void *)0), 0x41503232UL, ((void *)0),
    ((void *)0),
    ((void *)0),
    create_cgi_config,
    merge_cgi_config,
