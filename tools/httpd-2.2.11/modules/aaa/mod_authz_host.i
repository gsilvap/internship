...
# 118 "/usr/include/apr-1/apr_want.h"
...
# 119 "/usr/include/apr-1/apr_want.h" 2
# 31 "mod_authz_host.c" 2
...
static void *create_authz_host_dir_config(apr_pool_t *p, char *dummy)
{
    ...
    authz_host_dir_conf *conf =
        (authz_host_dir_conf *)memset(apr_palloc(p, sizeof(authz_host_dir_conf)), 0, sizeof(authz_host_dir_conf));
                             ...
                             const char *where_c)
{
    ...
    else if (!((rv = apr_ipsubnet_create(&a->x.ip, where, ((void *)0), cmd->pool)) == 22)
                                                                             ) {
        if (rv != 0) {
            apr_strerror(rv, msgbuf, sizeof msgbuf);
            return apr_pstrdup(cmd->pool, msgbuf);
...
{
    { "order", { .take1=order }, ((void *)0), 1, TAKE1, "'allow,deny', 'deny,allow', or 'mutual-failure'" }
                                                                    ,
    { "allow", { .take2=allow_cmd }, &its_an_allow, 1, ITERATE2, "'from' followed by hostnames or IP-address wildcards" }
                                                                            ,
    { "deny", { .take2=allow_cmd }, ((void *)0), 1, ITERATE2, "'from' followed by hostnames or IP-address wildcards" }
                                                                            ,
    {((void *)0)}
};
...
static int find_allowdeny(request_rec *r, apr_array_header_t *a, int method)
{
    ...
    apr_int64_t mmask = (((apr_int64_t)1) << method);
    int i;
    int gothost = 0;
    const char *remotehost = ((void *)0);
    ...
    for (i = 0; i < a->nelts; ++i) {
        ...
        case T_HOST:
            if (!gothost) {
                ...
                if ((remotehost == ((void *)0)) || remotehost_is_ip) {
                    gothost = 1;
                }
                else {
                    gothost = 2;
...
static int check_dir_access(request_rec *r)
{
    ...
    authz_host_dir_conf *a = (authz_host_dir_conf *)
        (((void **)(r->per_dir_config))[(&authz_host_module)->module_index]);
    ...
    if (ret == 403
        && (ap_satisfies(r) != 1 || !ap_some_auth_required(r))) {
        ap_log_rerror("mod_authz_host.c",311, 3, 0, r,
...
static void register_hooks(apr_pool_t *p)
{
    ...
    ap_hook_access_checker(check_dir_access,((void *)0),((void *)0),10);
}

module authz_host_module =
{
    20051115, 21, -1, "mod_authz_host.c", ((void *)0), ((void *)0), 0x41503232UL, ((void *)0),
