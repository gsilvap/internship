...
static char *etag_uint64_to_hex(char *next, apr_uint64_t u)
{
    ...
    do {
        ...
        if (next_digit) {
            *next++ = "0123456789abcdef"[next_digit];
            printing = 1;
        }
        else if (printing) {
            *next++ = "0123456789abcdef"[next_digit];
    ...
    *next++ = "0123456789abcdef"[u & (apr_uint64_t)0xf];
    return next;
}
# 64 "http_etag.c"
char * ap_make_etag(request_rec *r, int force_weak)
{
    ...
    if (etag_bits == 0) {
        etag_bits = ((1 << 1) | (1 << 2) | (1 << 3));
    }
# 103 "http_etag.c"
    if ((r->request_time - r->mtime > (1 * 1000000L)) &&
        !force_weak) {
        weak = ((void *)0);
        weak_len = 0;
    }
