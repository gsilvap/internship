...
void ap_die(int type, request_rec *r)
{
    ...
    if (type == -3) {
        ...
        if (next) {
            ap_log_rerror("http_request.c",101, 3, 0, r,
    ...
    if (r->status != 200) {
        ...
        while (r_1st_err->prev && (r_1st_err->prev->status != 200))
...
# 137 "http_request.c"
    ...
    if ((((r->status) == 400) || ((r->status) == 408) || ((r->status) == 411) || ((r->status) == 413) || ((r->status) == 414) || ((r->status) == 500) || ((r->status) == 503) || ((r->status) == 501))) {
    ...
    if (custom_response && custom_response[0] != '"') {
        ...
        if (ap_is_url(custom_response)) {
            ...
            recursive_error = 500;
            ap_log_rerror("http_request.c",210, 3, 0, r,
...
# 276 "http_request.c"
    if (ap_extended_status)
        ap_time_process_request(r->connection->sbh, 1);
...
# 306 "http_request.c"
    ...
    ap_update_child_status(r->connection->sbh, 6, r);
    ap_run_log_transaction(r);
    if (ap_extended_status)
        ap_time_process_request(r->connection->sbh, 2);
}
...
static apr_table_t *rename_original_env(apr_pool_t *p, apr_table_t *t)
{
    ...
    for (i = 0; i < env_arr->nelts; ++i) {
        if (!elts[i].key)
            continue;
        apr_table_setn(new, apr_pstrcat(p, "REDIRECT_", elts[i].key, ((void *)0)),
                  elts[i].val);
    }
...
static request_rec *internal_internal_redirect(const char *new_uri,
    ...
    new = (request_rec *) memset(apr_palloc(r->pool, sizeof(request_rec)), 0, sizeof(request_rec));
...
void ap_allow_methods(request_rec *r, int reset, ...)
{
    ...
    __builtin_va_start(methods,reset);
    while ((method = __builtin_va_arg(methods,const char *)) != ((void *)0)) {
...
void ap_allow_standard_methods(request_rec *r, int reset, ...)
{
    ...
    mask = 0;
    __builtin_va_start(methods,reset);
    while ((method = __builtin_va_arg(methods,int)) != -1) {
        mask |= (((apr_int64_t)1) << method);
    }
    __builtin_va_end(methods);
