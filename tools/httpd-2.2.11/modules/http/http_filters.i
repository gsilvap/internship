    ...
    if (!ctx) {
        ...
        if (tenc) {
            if (!strcasecmp(tenc, "chunked")) {
                ...
                ap_log_rerror("http_filters.c",262, 3, 0, f->r,
                              "Unknown Transfer-Encoding: %s", tenc);
                return bail_out_on_error(ctx, f, 501);
            }
            else {
                ap_log_rerror("http_filters.c",267, 4, 0, f->r,
        ...
        if (lenp && !tenc) {
            ...
            if (apr_strtoff(&ctx->remaining, lenp, &endstr, 10)
                ...
                ctx->remaining = 0;
                ap_log_rerror("http_filters.c",285, 3, 0, f->r,
            ...
            if (ctx->limit && ctx->limit < ctx->remaining) {
                ap_log_rerror("http_filters.c",295, 3, 0, f->r,
                          "Requested content-length of %" "ld"
                          " is larger than the configured limit"
                          " of %" "ld", ctx->remaining, ctx->limit);
                return bail_out_on_error(ctx, f, 413);
            }
        }
# 314 "http_filters.c"
    ...
    if (ctx->limit) {
        ...
        if (ctx->limit < ctx->limit_used) {
            ap_log_rerror("http_filters.c",541, 3, 0, f->r,
                          "Read content-length of %" "ld"
                          " is larger than the configured limit"
                          " of %" "ld", ctx->limit_used, ctx->limit);
            apr_brigade_cleanup(bb);
...
static long get_chunk_size(char *b)
{
    ...
    if ((((*__ctype_b_loc ())[(int) ((((unsigned char)(*b))))] & (unsigned short int) _ISxdigit)) && (chunkbits <= 0)) {
...
{
    char tmp[] = "X-Pad: avoid browser bug" "\015\012";
    char crlf[] = "\015\012";
    apr_off_t len;
    apr_size_t buflen;
    ...
    if (len >= 255 && len <= 257) {
        buflen = strlen(tmp);
        ;
        apr_brigade_write(bb, ((void *)0), ((void *)0), tmp, buflen);
    }
    buflen = strlen(crlf);
    ;
...
int ap_setup_client_block(request_rec *r, int read_policy)
{
    ...
    if (tenc) {
        if (strcasecmp(tenc, "chunked")) {
            ap_log_rerror("http_filters.c",1446, 3, 0, r,
                          "Unknown Transfer-Encoding %s", tenc);
            return 501;
        }
        if (r->read_body == 1) {
            ap_log_rerror("http_filters.c",1451, 3, 0, r,
        ...
        if (apr_strtoff(&r->remaining, lenp, &endstr, 10)
            ...
            r->remaining = 0;
            ap_log_rerror("http_filters.c",1464, 3, 0, r,
                          "Invalid Content-Length");
            return 400;
        }
    }
    ...
    if ((r->read_body == 0)
        ...
        ap_log_rerror("http_filters.c",1472, 3, 0, r,
