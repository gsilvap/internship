...
# 24 "/usr/include/apr-1/apr_strmatch.h"
# 1 "/usr/include/apr-1/apu.h" 1
# 25 "/usr/include/apr-1/apr_strmatch.h" 2
...
{
    sei_cfg_rec *a = memset(apr_palloc(p, sizeof(sei_cfg_rec)), 0, sizeof(sei_cfg_rec));
...
static int is_header_regex(apr_pool_t *p, const char* name)
{
    ...
    ap_regex_t *preg = ap_pregcomp(p, "^[-A-Za-z0-9_]*$",
                                   ((0) | (0) ));
    ((preg != ((void *)0)) ? (void)0 : ap_log_assert("preg != NULL","mod_setenvif.c",176));

    if (ap_regexec(preg, name, 0, ((void *)0), 0)) {
        return 1;
    }
                                     ...
                                     char *fname, const char *args)
{
    ...
    sconf = (cmd->path != ((void *)0))
      ? (sei_cfg_rec *) mconfig
      : (sei_cfg_rec *) (((void **)(cmd->server->module_config))[(&setenvif_module)->module_index])
                                                                ;
    ...
    icase = cmd->info == ((void *)(&setenvif_module));
    if (i < 0
        ...
        if ((simple_pattern = non_regex_pattern(cmd->pool, regex))) {
            ...
            if (new->pattern == ((void *)0)) {
                return apr_pstrcat(cmd->pool, cmd->cmd->name,
                                   " pattern could not be compiled.", ((void *)0));
            }
            new->preg = ((void *)0);
        }
        else {
            ...
            if (new->preg == ((void *)0)) {
                return apr_pstrcat(cmd->pool, cmd->cmd->name,
                                   " regex could not be compiled.", ((void *)0));
            }
            new->pattern = ((void *)0);
        }
...
{
    { "SetEnvIf", { .raw_args=add_setenvif }, ((void *)0), 4, RAW_ARGS, "A header-name, regex and a list of variables." }
                                                                     ,
    { "SetEnvIfNoCase", { .raw_args=add_setenvif }, ((void *)(&setenvif_module)), 4, RAW_ARGS, "a header-name, regex and a list of variables." }
                                                                     ,
    { "BrowserMatch", { .raw_args=add_browser }, ((void *)0), 4, RAW_ARGS, "A browser regex and a list of variables." }
                                                                ,
    { "BrowserMatchNoCase", { .raw_args=add_browser }, ((void *)(&setenvif_module)), 4, RAW_ARGS, "A browser regex and a list of variables." }
...
static int match_headers(request_rec *r)
{
    ...
    if (!(((void **)(r->request_config))[(&setenvif_module)->module_index])) {
        ((((void **)(r->request_config))[(&setenvif_module)->module_index]) = ("setenvif-phase-flag"))
                                                ;
    ...
    for (i = 0; i < sconf->conditionals->nelts; ++i) {
            ...
            case SPECIAL_NOT:
                if (b->pnamereg) {
                    ...
                    for (j = 0; j < arr->nelts; ++j) {
                        if (!ap_regexec(b->pnamereg, elts[j].key, 0, ((void *)0), 0)) {
                            val = elts[j].val;
                        }
                    }
                }
        ...
        if ((b->pattern && (*((b->pattern)->compare))((b->pattern), (val), (val_len))) ||
            (!b->pattern && !ap_regexec(b->preg, val, 10, regm,
                                        0))) {
...
{
    ap_hook_header_parser(match_headers, ((void *)0), ((void *)0), 10);
    ap_hook_post_read_request(match_headers, ((void *)0), ((void *)0), 10);
}

module setenvif_module =
{
    20051115, 21, -1, "mod_setenvif.c", ((void *)0), ((void *)0), 0x41503232UL, ((void *)0),
