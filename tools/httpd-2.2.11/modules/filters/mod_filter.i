...
# 49 "/usr/include/apr-1/apr_want.h"
...
# 50 "/usr/include/apr-1/apr_want.h" 2
...
# 29 "/usr/include/apr-1/apr_lib.h"
...
char * apr_strerror(apr_status_t statcode, char *buf,
                                 apr_size_t bufsize);
# 30 "/usr/include/apr-1/apr_lib.h" 2
...
# 36 "/usr/include/apr-1/apr_lib.h" 2
...
# 50 "/usr/include/apr-1/apr_strings.h"
                         ...
                         apr_bucket_brigade *bb)
{
    ...
    case 0:
        return;
    case 1:
        ap_log_cerror("mod_filter.c",117, 7, 0, c, "%s", fname);
        for (b = (&(bb)->list)->next;
             b != (struct apr_bucket *)((char *)(&(&(bb)->list)->next) - ((long) (((char *) (&(((struct apr_bucket*)((void *)0))->link))) - ((char *) ((void *)0)))));
             b = ((b))->link.next) {

            ap_log_cerror("mod_filter.c",122, 7, 0, c,
                          "%s: type: %s, length: %" "lu",
...
static int filter_init(ap_filter_t *f)
{
    ...
    harness_ctx *fctx = memset(apr_palloc(f->r->pool, sizeof(harness_ctx)), 0, sizeof(harness_ctx));
    for (p = filter->providers; p; p = p->next) {
        if (p->frec->filter_init_func == filter_init) {
            ap_log_cerror("mod_filter.c",141, 3, 0, f->c,
                          "Chaining of FilterProviders not supported");
            return 500;
        }
        else if (p->frec->filter_init_func) {
            f->ctx = ((void *)0);
            if ((err = p->frec->filter_init_func(f)) != 0) {
                ap_log_cerror("mod_filter.c",148, 3, 0, f->c,
                              "filter_init for %s failed", p->frec->name);
                return err;
            }
            if (f->ctx != ((void *)0)) {

                pctx = memset(apr_palloc(f->r->pool, sizeof(provider_ctx)), 0, sizeof(provider_ctx));
                pctx->provider = p;
                pctx->ctx = f->ctx;
...
static int filter_lookup(ap_filter_t *f, ap_filter_rec_t *filter)
{
    ...
    provider_ctx *pctx;
    mod_filter_ctx *rctx = (((void **)(r->request_config))[(&filter_module)->module_index])
                                                               ;
    ...
    for (provider = filter->providers; provider; provider = provider->next) {
        ...
        if (!str) {
            ...
            case STRING_CONTAINS:
                ...
                break;
            case REGEX_MATCH:
                if (ap_regexec(provider->match.regex, str, 0, ((void *)0), 0)
                    == AP_REG_NOMATCH) {
                    match = 0;
                }
                break;
...
static apr_status_t filter_harness(ap_filter_t *f, apr_bucket_brigade *bb)
{
    ...
    if (!ctx->func) {
        ...
        if (f->r->proxyreq) {
            if (filter->proto_flags & 0x8) {
                ap_remove_output_filter(f);
                return ap_pass_brigade(f->next, bb);
            }

            if (filter->proto_flags & 0x20) {
                                   ...
                                   const char *pname, const char *proto)
{
    ...
    ap_filter_provider_t *provider = ((void *)0);
    ...
    for (arg = apr_strtok(apr_pstrdup(cmd->pool, proto), sep, &tok);
         arg; arg = apr_strtok(((void *)0), sep, &tok)) {
                                  ...
                                  const char *place)
{
    ...
    filter = memset(apr_palloc(cmd->pool, sizeof(ap_filter_rec_t)), 0, sizeof(ap_filter_rec_t));
    apr_hash_set(cfg->live_filters, fname, (-1), filter);
...
static const char *filter_provider(cmd_parms *cmd, void *CFG, const char *args)
{
    ...
    frec = apr_hash_get(cfg->live_filters, fname, (-1));


    if (!frec) {
        c = filter_declare(cmd, CFG, fname, ((void *)0));
        if ( c ) {
            return c;
        }
        frec = apr_hash_get(cfg->live_filters, fname, (-1));
    ...
    case '/':
        ...
        provider->match.regex = ap_pregcomp(cmd->pool,
                                            apr_pstrndup(cmd->pool,
                                                         ...
                                                         rxend-match),
                                            flags);
        if (provider->match.regex == ((void *)0)) {
            return "Bad regexp";
        }
        break;
    case '*':
...
static const char *filter_chain(cmd_parms *cmd, void *CFG, const char *arg)
{
    ...
    switch (arg[0]) {
    case '+':
        p = memset(apr_palloc(cmd->pool, sizeof(mod_filter_chain)), 0, sizeof(mod_filter_chain));
        p->fname = arg+1;
        if (cfg->chain) {
    ...
    case '=':
        ...
        p = memset(apr_palloc(cmd->pool, sizeof(mod_filter_chain)), 0, sizeof(mod_filter_chain));
        p->fname = ((void *)0);
        p->next = memset(apr_palloc(cmd->pool, sizeof(mod_filter_chain)), 0, sizeof(mod_filter_chain));
        p->next->fname = arg+1;
        cfg->chain = p;
        break;
...
static void filter_insert(request_rec *r)
{
    mod_filter_chain *p;
    ap_filter_rec_t *filter;
    mod_filter_cfg *cfg = (((void **)(r->per_dir_config))[(&filter_module)->module_index])
                                                              ;

    int ranges = 1;
    mod_filter_ctx *ctx = memset(apr_palloc(r->pool, sizeof(mod_filter_ctx)), 0, sizeof(mod_filter_ctx));
    ((((void **)(r->request_config))[(&filter_module)->module_index]) = (ctx));
    ...
    for (p = cfg->chain; p; p = p->next) {
        filter = apr_hash_get(cfg->live_filters, p->fname, (-1));
        if (filter == ((void *)0)) {
            ap_log_rerror("mod_filter.c",767, 4, 0, r,
                          "Unknown filter %s not added", p->fname);
            continue;
        }
        ap_add_output_filter_handle(filter, ((void *)0), r, r->connection);
...
{
    ap_hook_insert_filter(filter_insert, ((void *)0), ((void *)0), 10);
}
...
static const command_rec filter_cmds[] = {
    { "FilterDeclare", { .take2=filter_declare }, ((void *)0), 2, TAKE12, "filter-name [, filter-type]" }
                                      ,

    { "FilterProvider", { .raw_args=filter_provider }, ((void *)0), 2, RAW_ARGS, "filter-name, provider-name, dispatch--criterion, dispatch-match" }
                                                                          ,
    { "FilterChain", { .take1=filter_chain }, ((void *)0), 2, ITERATE, "list of filter names with optional [+-=!@]" }
                                                     ,
    { "FilterTrace", { .take2=filter_debug }, ((void *)0), 128 | 64, TAKE2, "Debug level" }
                      ,

    { "FilterProtocol", { .take3=filter_protocol }, ((void *)0), 2, TAKE23, "filter-name [provider-name] protocol-args" }
                                                    ,
...
module filter_module = {
    20051115, 21, -1, "mod_filter.c", ((void *)0), ((void *)0), 0x41503232UL, ((void *)0),
    filter_config,
    filter_merge,
    ((void *)0),
    ((void *)0),
