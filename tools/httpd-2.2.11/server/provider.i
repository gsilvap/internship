...
static apr_hash_t *global_providers = ((void *)0);
static apr_hash_t *global_providers_names = ((void *)0);


static apr_status_t cleanup_global_providers(void *ctx)
{
    global_providers = ((void *)0);
    global_providers_names = ((void *)0);
    return 0;
}
                                              ...
                                              const void *provider)
{
    ...
    if (global_providers == ((void *)0)) {
        global_providers = apr_hash_make(pool);
        global_providers_names = apr_hash_make(pool);;
        apr_pool_cleanup_register(pool, ((void *)0), cleanup_global_providers,
    ...
    if (!provider_group_hash) {
        ...
        apr_hash_set(global_providers, provider_group, (-1),
                     provider_group_hash);
    ...
    if (!provider_version_hash) {
        ...
        apr_hash_set(provider_group_hash, provider_name, (-1),
                     provider_version_hash);

    }


    apr_hash_set(provider_version_hash, provider_version, (-1),
                 provider);
    ...
    if (!provider_group_hash) {
        provider_group_hash = apr_hash_make(pool);
        apr_hash_set(global_providers_names, provider_group, (-1),
                     provider_group_hash);
    ...
    if (!provider_version_hash) {
        provider_version_hash = apr_hash_make(pool);
        apr_hash_set(provider_group_hash, provider_version, (-1),
                     provider_version_hash);

    }


    apr_hash_set(provider_version_hash, provider_name, (-1),
                 provider_name);
                                              ...
                                              const char *provider_version)
{
    ...
    if (global_providers_names == ((void *)0)) {
        return ret;
    }
    ...
    for (hi = apr_hash_first(pool, h); hi; hi = apr_hash_next(hi)) {
        apr_hash_this(hi, (void *)&key, ((void *)0), (void *)&val);
