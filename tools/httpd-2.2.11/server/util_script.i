...
# 28 "/usr/include/apr-1/apr_lib.h"
# 1 "/usr/include/apr-1/apr.h" 1
# 29 "/usr/include/apr-1/apr_lib.h" 2
...
void ap_add_common_vars(request_rec *r)
{
    ...
    if (!(env_path = getenv("PATH"))) {
        env_path = "/bin:/usr/bin:/usr/ucb:/usr/bsd:/usr/local/bin";
    }
...
# 225 "util_script.c"
    ...
    host = ap_get_remote_host(c, r->per_dir_config, (0), ((void *)0));
    if (host) {
                                       ...
                                       void *getsfunc_data)
{
    ...
    while (1) {
        ...
        if ((*getsfunc) (w, 8192 - 1, getsfunc_data) == 0) {
            ap_log_rerror("util_script.c",434, 3|((0x07 + 1) * 2), 0, r,
...
# 524 "util_script.c"
        if (!(l = strchr(w, ':'))) {
            char malformed[(sizeof "malformed header from script. Bad header=") + 1
                           + 30];

            strcpy(malformed, "malformed header from script. Bad header=");
            strncat(malformed, w, 30);

            if (!buffer) {

                while ((*getsfunc) (w, 8192 - 1, getsfunc_data)) {
                    continue;
                }
            }

            ap_log_rerror("util_script.c",538, 3|((0x07 + 1) * 2), 0, r,
                          "%s: %s", malformed,
                          apr_filepath_name_get(r->filename));
            return 500;
        }
...
static int getsfunc_BRIGADE(char *buf, int len, void *arg)
{
    ...
    while ((dst < dst_end) && !done && !((e)->type == &apr_bucket_type_eos)) {
