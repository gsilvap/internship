...
# 34 "mpm_common.c" 2
# 1 "/usr/include/apr-1/apr_getopt.h" 1
# 42 "/usr/include/apr-1/apr_getopt.h"
...
# 30 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/ap_listen.h"
# 1 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/httpd.h" 1
# 31 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/ap_listen.h" 2
# 1 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/http_config.h" 1
...
# 60 "mpm_common.c" 2
# 70 "mpm_common.c"
static struct { apr_array_header_t *link_monitor; } _hooks;



void ap_hook_monitor(ap_HOOK_monitor_t *pf,const char * const *aszPre, const char * const *aszSucc,int nOrder) { ap_LINK_monitor_t *pHook; if(!_hooks.link_monitor) { _hooks.link_monitor=apr_array_make(apr_hook_global_pool,1,sizeof(ap_LINK_monitor_t)); apr_hook_sort_register("monitor",&_hooks.link_monitor); } pHook=apr_array_push(_hooks.link_monitor); pHook->pFunc=pf; pHook->aszPredecessors=aszPre; pHook->aszSuccessors=aszSucc; pHook->nOrder=nOrder; pHook->szName=apr_hook_debug_current; if(apr_hook_debug_enabled) apr_hook_debug_show("monitor",aszPre,aszSucc); } apr_array_header_t * ap_hook_get_monitor(void) { return _hooks.link_monitor; } int ap_run_monitor (apr_pool_t *p) { ap_LINK_monitor_t *pHook; int n; int rv = 0; ; ; if(_hooks.link_monitor) { pHook=(ap_LINK_monitor_t *)_hooks.link_monitor->elts; for(n=0 ; n < _hooks.link_monitor->nelts ; ++n) { ; rv=pHook[n].pFunc (p); ; if(rv != 0 && rv != -1) break; rv = 0; } } ; return rv; }
...
static int reclaim_one_pid(pid_t pid, action_t action)
{
    ...
    proc.pid = pid;
    waitret = apr_proc_wait(&proc, ((void *)0), ((void *)0), APR_NOWAIT);
    ...
    case SEND_SIGTERM:
        ...
        ap_log_error("mpm_common.c",146, 4,
                     0, ap_server_conf,
                     "child process %" "d"
                     " still did not exit, "
                     "sending a SIGTERM",
    ...
    case SEND_SIGKILL:
        ap_log_error("mpm_common.c",156, 3,
                     0, ap_server_conf,
                     "child process %" "d"
                     " still did not exit, "
                     "sending a SIGKILL",
...
# 173 "mpm_common.c"
    ...
    case GIVEUP:
        ...
        ap_log_error("mpm_common.c",181, 3,
                     0, ap_server_conf,
...
void ap_reclaim_child_processes(int terminate)
{
    ...
    do {
        ...
        for (i = 0; i < max_daemons; ++i) {
            pid_t pid = (ap_scoreboard_image->parent[i].pid);
...
# 350 "mpm_common.c"
    ...
    if (pg != getpgrp()) {
        ap_log_error("mpm_common.c",364, 1, 0, ap_server_conf,
...
# 430 "mpm_common.c"
    if ((why & APR_PROC_EXIT)) {
        ...
        if (status == 0xf) {
            ap_log_error("mpm_common.c",436, 1,
                         0, ap_server_conf,
                         "Child %" "d"
    ...
    if ((why & APR_PROC_SIGNAL)) {
        ...
        default:
            if ((why & APR_PROC_SIGNAL_CORE)) {
                ap_log_error("mpm_common.c",457, 5,
                             0, ap_server_conf,
                ...
                ap_log_error("mpm_common.c",465, 5,
                             0, ap_server_conf,
                             "child pid %ld exit signal %s (%d)",
                             (long)pid->pid, sigdesc, signum);
            }
...
{
# 488 "mpm_common.c"
    apr_status_t status = apr_socket_opt_set(s, 512, 1);

    if (status != 0) {
        ap_log_error("mpm_common.c",491, 4, status, ap_server_conf,
...
uid_t ap_uname2id(const char *name)
{
    ...
    if (!(ent = getpwnam(name))) {
        ap_log_error("mpm_common.c",506, ((0x07 + 1) * 4), 0, ((void *)0),
...
gid_t ap_gname2id(const char *name)
{
    ...
    if (!(ent = getgrnam(name))) {
        ap_log_error("mpm_common.c",524, ((0x07 + 1) * 4), 0, ((void *)0),
...
static apr_status_t pod_signal_internal(ap_pod_t *pod)
{
    ...
    if (rv != 0) {
        ap_log_error("mpm_common.c",632, 4, rv, ap_server_conf,
                     "write pipe_of_death");
    }
...
static apr_status_t dummy_connection(ap_pod_t *pod)
{
    ...
    rv = apr_socket_create(&sock, ap_listeners->bind_addr->family,
                           SOCK_STREAM, 0, p);
    if (rv != 0) {
        ap_log_error("mpm_common.c",661, 4, rv, ap_server_conf,
    ...
    if (rv != 0) {
        ap_log_error("mpm_common.c",675, 4, rv, ap_server_conf,
    ...
    if (rv != 0) {
        ...
        ap_log_error("mpm_common.c",694, log_level, rv, ap_server_conf,
...
const char *ap_pid_fname = ((void *)0);
...
const char *ap_lock_fname = ((void *)0);
                                   ...
                                   const char *arg)
{
    ...
    if ((rv = apr_stat(&finfo, fname, 0x00008000, cmd->pool)) != 0) {
        return apr_pstrcat(cmd->pool, "CoreDumpDirectory ", fname,
                           " does not exist", ((void *)0));
    }
    if (finfo.filetype != APR_DIR) {
        return apr_pstrcat(cmd->pool, "CoreDumpDirectory ", fname,
                           " is not a directory", ((void *)0));
    }
...
{
    if (kill(pid, sig) < 0) {
        ap_log_error("mpm_common.c",956, ((0x07 + 1) * 4), (*__errno_location ()), ((void *)0),
...
int ap_signal_server(int *exit_status, apr_pool_t *pconf)
{
    ...
    *exit_status = 0;
    ...
    if (rv != 0) {
        if (rv != 2) {
            ap_log_error("mpm_common.c",976, ((0x07 + 1) * 4), rv, ((void *)0),
                         "Error retrieving pid file %s", ap_pid_fname);
            ap_log_error("mpm_common.c",978, ((0x07 + 1) * 4), 0, ((void *)0),
...
void ap_mpm_rewrite_args(process_rec *process)
{
    ...
    *(const char **)apr_array_push(mpm_new_argv) = process->argv[0];
    ...
    optbuf[2] = '\0';
    while ((rv = apr_getopt(opt, "k:" "C:c:D:d:E:e:f:vVlLtSMh?X",
                            optbuf + 1, &optarg)) == 0) {
        switch(optbuf[1]) {
        case 'k':
            if (!dash_k_arg) {
                                    ...
                                    const char *arg)
{
    ...
    value = strtol(arg, ((void *)0), 0);
    if (value < 0 || (*__errno_location ()) == 34)
...
# 1224 "mpm_common.c"
    if (getpid() == parent_pid) {
        ap_log_error("mpm_common.c",1225, 5,
                     0, ap_server_conf,
...
apr_status_t ap_fatal_signal_setup(server_rec *s, apr_pool_t *in_pconf)
{
    ...
    if (sigaction(11, &sa, ((void *)0)) < 0)
        ap_log_error("mpm_common.c",1268, 4, (*__errno_location ()), s, "sigaction(SIGSEGV)");

    if (sigaction(7, &sa, ((void *)0)) < 0)
        ap_log_error("mpm_common.c",1271, 4, (*__errno_location ()), s, "sigaction(SIGBUS)");
    ...
    if (sigaction(6, &sa, ((void *)0)) < 0)
        ap_log_error("mpm_common.c",1279, 4, (*__errno_location ()), s, "sigaction(SIGABRT)");


    if (sigaction(4, &sa, ((void *)0)) < 0)
        ap_log_error("mpm_common.c",1283, 4, (*__errno_location ()), s, "sigaction(SIGILL)");


    if (sigaction(8, &sa, ((void *)0)) < 0)
        ap_log_error("mpm_common.c",1287, 4, (*__errno_location ()), s, "sigaction(SIGFPE)");
# 1311 "mpm_common.c"
    pconf = in_pconf;
