...
# 1 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/httpd.h" 1
# 36 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/util_ebcdic.h" 2
...
char * ap_md5_binary(apr_pool_t *p, const unsigned char *buf, int length)
{
    ...
    *r = '\0';
    ...
    return apr_pstrndup(p, result, 16*2);
}
...
char * ap_md5contextTo64(apr_pool_t *a, apr_md5_ctx_t *context)
{
    ...
    encodedDigest = (char *) memset(apr_palloc(a, 25 * sizeof(char)), 0, 25 * sizeof(char));
...
char * ap_md5digest(apr_pool_t *p, apr_file_t *infile)
{
    ...
    nbytes = sizeof(buf);
    while (apr_file_read(infile, buf, &nbytes) == 0) {
