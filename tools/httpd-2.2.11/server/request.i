...
{
    if (status == -1) {
        ap_log_rerror("request.c",88, 2, 0, r,
...
int ap_process_request_internal(request_rec *r)
{
    ...
    if (!r->proxyreq && r->parsed_uri.path) {
        core_dir_config *d;
        d = (((void **)(r->per_dir_config))[(&core_module)->module_index]);
        ...
        if (access_status) {
            if (access_status == 404) {
                if (! d->allow_encoded_slashes) {
                    ap_log_rerror("request.c",120, 6, 0, r,
...
static int resolve_symlink(char *d, apr_finfo_t *lfi, int opts, apr_pool_t *p)
{
    ...
    if (opts & 4) {
        if ((res = apr_stat(&fi, d, lfi->valid & ~(0x02000000
                                                 | 0x00000001), p))
                 != 0) {
            return 403;
        }
    ...
    if (!(lfi->valid & 0x00030000)) {
        if ((res = apr_stat(&fi, d,
                            lfi->valid | 0x00000001 | 0x00030000, p))
            != 0) {
            return 403;
        }
    }

    if ((res = apr_stat(&fi, d, lfi->valid & ~(0x02000000), p))
        != 0) {
        return 403;
    }
...
{
    core_dir_config *this_dir = (((void **)(sec))[(&core_module)->module_index]);
...
int ap_directory_walk(request_rec *r)
{
    ...
    core_server_config *sconf = (((void **)(r->server->module_config))[(&core_module)->module_index])
                                                                  ;
...
# 484 "request.c"
    if (r->filename == ((void *)0)) {
        ap_log_rerror("request.c",485, 6, 0, r,
    ...
    if ((rv = apr_filepath_merge(&entry_dir, ((void *)0), r->filename,
                                 0x04, r->pool))
                  != 0) {
        ap_log_rerror("request.c",498, 6, 0, r,
...
# 609 "request.c"
        this_dir = (((void **)(r->per_dir_config))[(&core_module)->module_index]);
        opts.opts = this_dir->opts;
        opts.add = this_dir->opts_add;
        opts.remove = this_dir->opts_remove;
        opts.override = this_dir->override;
...
# 624 "request.c"
        if ((r->finfo.filetype == APR_DIR) && r->path_info && *r->path_info)
        {
            if ((rv = apr_filepath_merge(&r->path_info, r->filename,
                                         ...
                                         0x01, r->pool))
                != 0) {
                ap_log_rerror("request.c",630, 3, rv, r,
...
# 689 "request.c"
        ...
        if (rv != 0) {
            ap_log_rerror("request.c",697, 3, rv, r,
...
# 720 "request.c"
        ...
        do {
            ...
            for (; sec_idx < num_sec; ++sec_idx) {
                ...
                entry_core = (((void **)(entry_config))[(&core_module)->module_index]);
...
# 946 "request.c"
            ...
            if (((rv) == 2)) {
                ...
                ap_log_rerror("request.c",958, 3, rv, r,
                ...
                ap_log_rerror("request.c",967, 3, rv, r,
            ...
            if (thisinfo.filetype == APR_LNK) {
                ...
                if ((res = resolve_symlink(r->filename, &thisinfo,
                                           opts.opts, r->pool)) != 0) {
                    ap_log_rerror("request.c",989, 3, 0, r,
            ...
            if (thisinfo.filetype == APR_REG ||
                ...
                ap_log_rerror("request.c",1006, 3, 0, r,
        ...
        for (; sec_idx < num_sec; ++sec_idx) {
            ...
            core_dir_config *entry_core;
            entry_core = (((void **)(sec_ent[sec_idx]))[(&core_module)->module_index]);

            if (!entry_core->r) {
                continue;
            }

            if (ap_regexec(entry_core->r, r->filename, 0, ((void *)0), 0x08)) {
                continue;
            }
...
{
    ap_conf_vector_t *now_merged = ((void *)0);
    core_dir_config *dconf = (((void **)(r->per_dir_config))[(&core_module)->module_index])
                                                               ;
                                                 ...
                                                 ap_filter_t *next_filter)
{
    ...
    if (strncmp(rnew->filename, fdir, fdirlen) == 0
        ...
        && strchr(rnew->filename + fdirlen, '/') == ((void *)0)) {
        apr_status_t rv;
        if (ap_allow_options(rnew) & 4) {
            if (((rv = apr_stat(&rnew->finfo, rnew->filename,
                                0x00008170, rnew->pool)) != 0)
