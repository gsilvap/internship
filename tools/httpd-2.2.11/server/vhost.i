                                 ...
                                 apr_port_t default_port)
{
    ...
    if (strcmp(host, "*") == 0) {
        ...
        rv = apr_sockaddr_info_get(&my_addr, host, 0, port, 0, p);
        if (rv != 0) {
            ap_log_error("vhost.c",202, 3, rv, ((void *)0),
    ...
    do {
        sar = memset(apr_palloc(p, sizeof(server_addr_rec)), 0, sizeof(server_addr_rec));
        **paddr = sar;
        *paddr = &sar->next;
...
# 325 "vhost.c"
static __inline__ unsigned hash_inaddr(unsigned key)
{
    key ^= (key >> 16);
    return ((key >> 8) ^ key) % 256;
}

static __inline__ unsigned hash_addr(struct apr_sockaddr_t *sa)
{
    unsigned key;
...
static __inline__ ipaddr_chain *find_ipaddr(apr_sockaddr_t *sa)
{
    unsigned bucket;
    ipaddr_chain *trav;
...
static void dump_a_vhost(apr_file_t *f, ipaddr_chain *ic)
{
    ...
    if (ha->family == 2 &&
        ha->sa.sin.sin_addr.s_addr == 0xfffffffful) {
        len = apr_snprintf(buf, sizeof(buf), "_default_:%u",
                           ic->sar->host_port);
    }
    else if (ha->family == 2 &&
             ha->sa.sin.sin_addr.s_addr == ((in_addr_t) 0x00000000)) {
                                 ...
                                 ipaddr_chain *ic)
{
    ...
    if (ic->names || ic->server == ((void *)0)) {
        ...
        if (sar->host_port != ic->sar->host_port) {
            ...
            ap_log_error("vhost.c",488, 3, 0, main_s,
...
static void remove_unused_name_vhosts(server_rec *main_s, ipaddr_chain **pic)
{
    while (*pic) {
        ...
        if (ic->server == ((void *)0)) {
            ap_log_error("vhost.c",509, 4, 0, main_s,
...
void ap_fini_vhost_config(apr_pool_t *p, server_rec *main_s)
{
    ...
    *name_vhost_list_tail = ((void *)0);
    ...
    for (sar = name_vhost_list; sar; sar = sar->next) {
        ...
        ipaddr_chain *ic = new_ipaddr_chain(p, ((void *)0), sar);
    ...
    for (s = main_s->next; s; s = s->next) {
        ...
        for (sar = s->addrs; sar; sar = sar->next) {
            ...
            if ((sar->host_addr->family == 2 &&
                 sar->host_addr->sa.sin.sin_addr.s_addr == 0xfffffffful)
        ...
        if (!s->server_hostname) {
            if (has_default_vhost_addr) {
                ...
                if (rv == 0) {
                    ...
                    ap_log_error("vhost.c",655, 3, rv, main_s,
...
static void fix_hostname(request_rec *r)
{
    ...
    if (r->hostname[0] != '[') {
        for (dst = host; *dst; dst++) {
            if ((((*__ctype_b_loc ())[(int) ((((unsigned char)(*dst))))] & (unsigned short int) _ISlower))) {
            ...
            else if ((((*__ctype_b_loc ())[(int) ((((unsigned char)(*dst))))] & (unsigned short int) _ISupper))) {
                *dst = (tolower(((unsigned char)(*dst))));
            }
            else if (*dst == '/' || *dst == '\\') {
                goto bad;
            }
...
bad:
    r->status = 400;
    ap_log_rerror("vhost.c",767, 3, 0, r,
