...
# 45 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/http_request.h"
...
# 46 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/http_request.h" 2
...
# 31 "/usr/include/apr-1/apr_thread_proc.h"
...
# 29 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/ap_mpm.h"
# 1 "/usr/include/apr-1/apr_thread_proc.h" 1
# 30 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/ap_mpm.h" 2
...
static struct { apr_array_header_t *link_create_connection; apr_array_header_t *link_process_connection; apr_array_header_t *link_pre_connection; } _hooks;




void ap_hook_create_connection(ap_HOOK_create_connection_t *pf,const char * const *aszPre, const char * const *aszSucc,int nOrder) { ap_LINK_create_connection_t *pHook; if(!_hooks.link_create_connection) { _hooks.link_create_connection=apr_array_make(apr_hook_global_pool,1,sizeof(ap_LINK_create_connection_t)); apr_hook_sort_register("create_connection",&_hooks.link_create_connection); } pHook=apr_array_push(_hooks.link_create_connection); pHook->pFunc=pf; pHook->aszPredecessors=aszPre; pHook->aszSuccessors=aszSucc; pHook->nOrder=nOrder; pHook->szName=apr_hook_debug_current; if(apr_hook_debug_enabled) apr_hook_debug_show("create_connection",aszPre,aszSucc); } apr_array_header_t * ap_hook_get_create_connection(void) { return _hooks.link_create_connection; } conn_rec * ap_run_create_connection (apr_pool_t *p, server_rec *server, apr_socket_t *csd, long conn_id, void *sbh, apr_bucket_alloc_t *alloc) { ap_LINK_create_connection_t *pHook; int n; conn_rec * rv = ((void *)0); ; ; if(_hooks.link_create_connection) { pHook=(ap_LINK_create_connection_t *)_hooks.link_create_connection->elts; for(n=0 ; n < _hooks.link_create_connection->nelts ; ++n) { ; rv=pHook[n].pFunc (p, server, csd, conn_id, sbh, alloc); ; if(rv != ((void *)0)) break; } } ; return rv; }


void ap_hook_process_connection(ap_HOOK_process_connection_t *pf,const char * const *aszPre, const char * const *aszSucc,int nOrder) { ap_LINK_process_connection_t *pHook; if(!_hooks.link_process_connection) { _hooks.link_process_connection=apr_array_make(apr_hook_global_pool,1,sizeof(ap_LINK_process_connection_t)); apr_hook_sort_register("process_connection",&_hooks.link_process_connection); } pHook=apr_array_push(_hooks.link_process_connection); pHook->pFunc=pf; pHook->aszPredecessors=aszPre; pHook->aszSuccessors=aszSucc; pHook->nOrder=nOrder; pHook->szName=apr_hook_debug_current; if(apr_hook_debug_enabled) apr_hook_debug_show("process_connection",aszPre,aszSucc); } apr_array_header_t * ap_hook_get_process_connection(void) { return _hooks.link_process_connection; } int ap_run_process_connection (conn_rec *c) { ap_LINK_process_connection_t *pHook; int n; int rv = -1; ; ; if(_hooks.link_process_connection) { pHook=(ap_LINK_process_connection_t *)_hooks.link_process_connection->elts; for(n=0 ; n < _hooks.link_process_connection->nelts ; ++n) { ; rv=pHook[n].pFunc (c); ; if(rv != -1) break; } } ; return rv; }
void ap_hook_pre_connection(ap_HOOK_pre_connection_t *pf,const char * const *aszPre, const char * const *aszSucc,int nOrder) { ap_LINK_pre_connection_t *pHook; if(!_hooks.link_pre_connection) { _hooks.link_pre_connection=apr_array_make(apr_hook_global_pool,1,sizeof(ap_LINK_pre_connection_t)); apr_hook_sort_register("pre_connection",&_hooks.link_pre_connection); } pHook=apr_array_push(_hooks.link_pre_connection); pHook->pFunc=pf; pHook->aszPredecessors=aszPre; pHook->aszSuccessors=aszSucc; pHook->nOrder=nOrder; pHook->szName=apr_hook_debug_current; if(apr_hook_debug_enabled) apr_hook_debug_show("pre_connection",aszPre,aszSucc); } apr_array_header_t * ap_hook_get_pre_connection(void) { return _hooks.link_pre_connection; } int ap_run_pre_connection (conn_rec *c, void *csd) { ap_LINK_pre_connection_t *pHook; int n; int rv = 0; ; ; if(_hooks.link_pre_connection) { pHook=(ap_LINK_pre_connection_t *)_hooks.link_pre_connection->elts; for(n=0 ; n < _hooks.link_pre_connection->nelts ; ++n) { ; rv=pHook[n].pFunc (c, csd); ; if(rv != 0 && rv != -1) break; rv = 0; } } ; return rv; }
# 69 "connection.c"
void ap_flush_conn(conn_rec *c)
{
    apr_bucket_brigade *bb;
...
void ap_lingering_close(conn_rec *c)
{
    ...
    apr_size_t nbytes;
    apr_time_t timeup = 0;
    apr_socket_t *csd = (((void **)(c->conn_config))[(&core_module)->module_index]);

    if (!csd) {
        return;
    }

    ap_update_child_status(c->sbh, 8, ((void *)0));
# 123 "connection.c"
    ap_flush_conn(c);
    ...
    if (apr_socket_shutdown(csd, APR_SHUTDOWN_WRITE) != 0
        || c->aborted) {
        apr_socket_close(csd);
        return;
    }
    ...
    apr_socket_timeout_set(csd, ((apr_time_t)(2) * 1000000L));
    ...
    do {
        ...
        if (timeup == 0) {
            ...
            timeup = apr_time_now() + ((apr_time_t)(30) * 1000000L);
            continue;
        }
