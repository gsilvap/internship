...
# 38 "scoreboard.c" 2
 ...
 scoreboard *ap_scoreboard_image = ((void *)0);
 const char *ap_scoreboard_fname = ((void *)0);
...
static
        ...
        apr_shm_t *ap_scoreboard_shm = ((void *)0);



static struct { apr_array_header_t *link_pre_mpm; } _hooks;



void ap_hook_pre_mpm(ap_HOOK_pre_mpm_t *pf,const char * const *aszPre, const char * const *aszSucc,int nOrder) { ap_LINK_pre_mpm_t *pHook; if(!_hooks.link_pre_mpm) { _hooks.link_pre_mpm=apr_array_make(apr_hook_global_pool,1,sizeof(ap_LINK_pre_mpm_t)); apr_hook_sort_register("pre_mpm",&_hooks.link_pre_mpm); } pHook=apr_array_push(_hooks.link_pre_mpm); pHook->pFunc=pf; pHook->aszPredecessors=aszPre; pHook->aszSuccessors=aszSucc; pHook->nOrder=nOrder; pHook->szName=apr_hook_debug_current; if(apr_hook_debug_enabled) apr_hook_debug_show("pre_mpm",aszPre,aszSucc); } apr_array_header_t * ap_hook_get_pre_mpm(void) { return _hooks.link_pre_mpm; } int ap_run_pre_mpm (apr_pool_t *p, ap_scoreboard_e sb_type) { ap_LINK_pre_mpm_t *pHook; int n; int rv = 0; ; ; if(_hooks.link_pre_mpm) { pHook=(ap_LINK_pre_mpm_t *)_hooks.link_pre_mpm->elts; for(n=0 ; n < _hooks.link_pre_mpm->nelts ; ++n) { ; rv=pHook[n].pFunc (p, sb_type); ; if(rv != 0 && rv != -1) break; rv = 0; } } ; return rv; }



static apr_OFN_ap_proxy_lb_workers_t
                                *proxy_lb_workers;
...
int ap_calc_scoreboard_size(void)
{
    ...
    if (!proxy_lb_workers)
        proxy_lb_workers = (apr_OFN_ap_proxy_lb_workers_t *)apr_dynamic_fn_retrieve("ap_proxy_lb_workers");
    if (proxy_lb_workers)
...
void ap_init_scoreboard(void *shared_score)
{
    ...
    ((more_storage == (char*)shared_score + scoreboard_size) ? (void)0 : ap_log_assert("more_storage == (char*)shared_score + scoreboard_size","scoreboard.c",137));
                                                ...
                                                const char *fname)
{
    ...
    if (rv != 0) {
        ap_log_error("scoreboard.c",159, 2, rv, ((void *)0),
...
static apr_status_t open_scoreboard(apr_pool_t *pconf)
{
    ...
    rv = apr_pool_create_ex(&global_pool, ((void *)0), ((void *)0), ((void *)0));
    if (rv != 0) {
        ap_log_error("scoreboard.c",184, 2, rv, ((void *)0),
    ...
    if (ap_scoreboard_fname) {
        ...
        if (!fname) {
            ap_log_error("scoreboard.c",195, 2, (20000 + 24), ((void *)0),
        ...
        rv = apr_shm_create(&ap_scoreboard_shm, scoreboard_size, ((void *)0),
                            global_pool);
        if ((rv != 0) && (rv != ((20000 + 50000) + 23))) {
            ap_log_error("scoreboard.c",206, 2, rv, ((void *)0),
...
int ap_create_scoreboard(apr_pool_t *p, ap_scoreboard_e sb_type)
{
...

    {
        ...
        if (sb_mem == ((void *)0)) {
            ap_log_error("scoreboard.c",309, 2, 0, ((void *)0),
    ...
    apr_pool_cleanup_register(p, ((void *)0), ap_cleanup_scoreboard, apr_pool_cleanup_null);
...
static void copy_request(char *rbuf, apr_size_t rbuflen, request_rec *r)
{
    ...
    if (r->parsed_uri.password == ((void *)0)) {
        p = r->the_request;
    }
    else {
        ...
        p = apr_pstrcat(r->pool, r->method, " ",
                        ...
                        (1U<<2)),
                        r->assbackwards ? ((void *)0) : " ", r->protocol, ((void *)0));
    }
...
int ap_update_child_status(ap_sb_handle_t *sbh, int status,
                                      request_rec *r)
{
    if (!sbh)
        return -1;
...
global_score * ap_get_scoreboard_global()
{
