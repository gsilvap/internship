...
# 29 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/http_protocol.h"
# 1 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/httpd.h" 1
# 30 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/http_protocol.h" 2
...
# 35 "/run/media/master/data/OneDrive/mei/internship/tools/httpd-2.2.11/include/http_protocol.h" 2
...
static struct { apr_array_header_t *link_header_parser; apr_array_header_t *link_pre_config; apr_array_header_t *link_post_config; apr_array_header_t *link_open_logs; apr_array_header_t *link_child_init; apr_array_header_t *link_handler; apr_array_header_t *link_quick_handler; apr_array_header_t *link_optional_fn_retrieve; apr_array_header_t *link_test_config; } _hooks;
# 79 "config.c"
...
{
    void *conf_vector = memset(apr_palloc(p, sizeof(void *) * (total_modules + 128)), 0, sizeof(void *) * (total_modules + 128))
                                                                           ;
    return conf_vector;
}
...
# 339 "config.c"
    ...
    if (result == -1 && r->handler && r->filename) {
        ap_log_rerror("config.c",377, 4, 0, r,
    ...
    if ((result != 0) && (result != -2) && (result != -1)
        ...
        if (!ignore) {
            ap_log_rerror("config.c",391, 3, 0, r,
...
static void rebuild_conf_hash(apr_pool_t *p, int add_prelinked)
{
    ...
    apr_pool_cleanup_register(p, ((void *)0), reload_conf_hash,
...
void ap_remove_module(module *m)
{
    ...
    if (modp == m) {
        ...
        if (!modp) {
            ...
            ap_log_error("config.c",586, 3, 0, ((void *)0),
...
const char * ap_setup_prelinked_modules(process_rec *process)
{
    ...
    ap_loaded_modules = (module **)apr_palloc(process->pool,
        sizeof(module *) * (total_modules + 128 + 1));

    if (ap_loaded_modules == ((void *)0)) {
                              ...
                              void *mconfig, const char *args)
{
    ...
    if ((parms->override & cmd->req_override) == 0)
        return apr_pstrcat(parms->pool, cmd->name, " not allowed here", ((void *)0));
                                              ...
                                              char *orig_directive)
{
    ...
    l = apr_palloc(temp_pool, 8192);

    bracket = apr_pstrcat(p, orig_directive + 1, ">", ((void *)0));
    while (!(ap_cfg_getline(l, 8192, parms->config_file))) {
        if (!memcmp(l, "</", 2)
        ...
        if (sub_tree == ((void *)0) && curr_parent != ((void *)0)) {
            sub_tree = *curr_parent;
        }

        if (sub_tree == ((void *)0) && current != ((void *)0)) {
            sub_tree = *current;
        }
    }
                                      ...
                                      ap_conf_vector_t *section_vector)
{
    ...
    for ( ; ml != ((void *)0); ml = ml->next) {
        ...
        if (retval != ((void *)0) && strcmp(retval, "\a\b") != 0) {
                                         ...
                                         ap_directive_t **conftree)
{
    ...
    char *l = apr_palloc (temp_pool, 8192);
    const char *errmsg;
...
{((void *)0), 0, -1, ((void *)0), ((void *)0), ((void *)0), ((void *)0), ((void *)0), ((void *)0), ((void *)0), ((void *)0), ((void *)0)};
...
const char * ap_soak_end_container(cmd_parms *cmd, char *directive)
{
    ...
    while(!(ap_cfg_getline(l, 8192, cmd->config_file))) {
                                          ...
                                          apr_pool_t *ptemp)
{
    ...
    parms.override_opts = (1|2|4|8) | 32 | 64 | 128;
                                                    ...
                                                    apr_pool_t *ptemp)
{
    ...
    if ((ap_server_pre_read_config->nelts
        ...
        && !(strcmp(fname, ap_server_root_relative(p, "conf/httpd.conf")))) {
        apr_finfo_t finfo;

        if (apr_stat(&finfo, fname, 0x00000001 | 0x00008000, p) != 0)
            return ((void *)0);
    }
    ...
    if (!apr_fnmatch_test(fname)) {
        ...
        fnames *fnew;
        apr_status_t rv;
        char *path = apr_pstrdup(p, fname), *pattern = ((void *)0);
                                       ...
                                       apr_pool_t *ptemp)
{
    ...
    if (errmsg) {
        ap_log_perror("config.c",1763, ((0x07 + 1) * 4), 0, p,
                     "Syntax error on line %d of %s:",
                     parms.err_directive->line_num,
                     parms.err_directive->filename);
        ap_log_perror("config.c",1767, ((0x07 + 1) * 4), 0, p,
                     "%s", errmsg);
        return 500;
    }
                                       ...
                                       const char *d, const char *access_name)
{
    ...
    while (access_name[0]) {
        ...
        if (status == 0) {
            ...
            if (errmsg) {
                ap_log_rerror("config.c",1828, 1, 0, r,
                              "%s: %s", filename, errmsg);
                return 500;
            }
            ...
            *result = dc;
            ...
            if (!((status) == 2)
                && !((status) == 20)) {
                ap_log_rerror("config.c",1839, 2, status, r,
...
static server_rec *init_server_config(process_rec *process, apr_pool_t *p)
{
    ...
    s->timeout = ((apr_time_t)(300) * 1000000L);
    ...
    rv = apr_sockaddr_info_get(&s->addrs->host_addr,
                               ((void *)0), 2, 0, 0, p);
    ((rv == 0) ? (void)0 : ap_log_assert("rv == APR_SUCCESS","config.c",1978));
                                       ...
                                       ap_directive_t **conftree)
{
    ...
    error = process_command_config(s, ap_server_pre_read_config, conftree,
                                   p, ptemp);
    if (error) {
        ap_log_error("config.c",2005, ((0x07 + 1) * 4)|2, 0, ((void *)0), "%s: %s",
    ...
    if (!confname) {
        ap_log_error("config.c",2016, ((0x07 + 1) * 4)|2,
                     (20000 + 24), ((void *)0), "Invalid config file path %s",
                     filename);
        return ((void *)0);
    }
    ...
    if (error) {
        ap_log_error("config.c",2024, ((0x07 + 1) * 4)|2, 0, ((void *)0),
    ...
    if (error) {
        ap_log_error("config.c",2033, ((0x07 + 1) * 4)|2, 0, ((void *)0), "%s: %s",
                     ap_server_argv0, error);
        return ((void *)0);
    }
