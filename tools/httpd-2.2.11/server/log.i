...
static struct { apr_array_header_t *link_error_log; } _hooks;
                                               ...
                                               const char *fname)
{
    ...
    if (!filename) {
        ap_log_error("log.c",208, ((0x07 + 1) * 4)|2,
                     (20000 + 24), ((void *)0), "Invalid -E error log file %s",
                     fname);
        return (20000 + 24);
    }
    if ((rc = apr_file_open(&stderr_file, filename,
                            ...
                            0x0FFF, p)) != 0) {
        ap_log_error("log.c",216, ((0x07 + 1) * 4), rc, ((void *)0),
...
# 242 "log.c"
    ...
    if (rc != 0) {
        ap_log_error("log.c",249, 2, rc, ((void *)0),
...
static void log_child_errfn(apr_pool_t *pool, apr_status_t err,
                            const char *description)
{
    ap_log_error("log.c",258, 3, err, ((void *)0),
                 "%s", description);
}
                     ...
                     apr_file_t **fpin, int dummy_stderr)
{
    ...
    if (((rc = apr_procattr_create(&procattr, p)) == 0)
        ...
        if ((rc = apr_file_open_stdout(&outfile, p)) == 0) {
            rc = apr_procattr_child_out_set(procattr, outfile, ((void *)0));
            if (dummy_stderr)
                rc = apr_procattr_child_err_set(procattr, outfile, ((void *)0));
            else if ((rc = apr_file_open_stderr(&errfile, p)) == 0)
...
static int open_error_log(server_rec *s, int is_main, apr_pool_t *p)
{
    ...
    if (*s->error_fname == '|') {
        ...
        if (rc != 0) {
            ap_log_error("log.c",335, ((0x07 + 1) * 4), rc, ((void *)0),
        ...
        if (!fname) {
            ap_log_error("log.c",368, ((0x07 + 1) * 4), (20000 + 24), ((void *)0),
        ...
        if ((rc = apr_file_open(&s->error_log, fname,
                               0x00008 | 0x00002 | 0x00004 | 0x04000,
                               0x0FFF, p)) != 0) {
            ap_log_error("log.c",376, ((0x07 + 1) * 4), rc, ((void *)0),
                         "%s: could not open error log file %s.",
                         ap_server_argv0, fname);
            return -2;
        }
                 ...
                 apr_pool_t *ptemp, server_rec *s_main)
{
    ...
    int replace_stderr;
# 403 "log.c"
    apr_pool_cleanup_register(p, ((void *)0), clear_handle_list,
    ...
    if (s_main->error_log) {
        ...
        if (rv != 0) {
            ap_log_error("log.c",427, 2, rv, s_main,
...
# 446 "log.c"
    ...
    if (replace_stderr && freopen("/dev/null", "w", stderr) == ((void *)0)) {
        ap_log_error("log.c",456, 2, (*__errno_location ()), s_main,
...
# 552 "log.c"
    ...
    if (logf && ((level & ((0x07 + 1) * 4)) != ((0x07 + 1) * 4))) {
        errstr[0] = '[';
    ...
    if ((level & ((0x07 + 1) * 4)) != ((0x07 + 1) * 4)) {
        len += apr_snprintf(errstr + len, 8192 - len,
                            "[%s] ", priorities[level_and_mask].t_name);
    }


    if (file && level_and_mask == 7) {
# 603 "log.c"
        const char *p;
    ...
    if (status != 0) {
        if (status < ((((20000 + 50000) + 50000) + (50000 * 10)) + 50000)) {
            ...
            len += apr_snprintf(errstr + len, 8192 - len,
                                "(EAI %d)", status - ((((20000 + 50000) + 50000) + (50000 * 10)) + 50000));
        }
        else if (status < 100000 + (((((20000 + 50000) + 50000) + (50000 * 10)) + 50000) + 50000)) {
            len += apr_snprintf(errstr + len, 8192 - len,
                                "(OS %d)", status - (((((20000 + 50000) + 50000) + (50000 * 10)) + 50000) + 50000));
        }
        else {
            len += apr_snprintf(errstr + len, 8192 - len,
                                "(os 0x%08x)", status - (((((20000 + 50000) + 50000) + (50000 * 10)) + 50000) + 50000));
        }
        apr_strerror(status, errstr + len, 8192 - len);
    ...
    if (apr_vsnprintf(scratch, 8192 - len, fmt, args)) {
    ...
    if ( r && (referer = apr_table_get(r->headers_in, "Referer"))
        ...
        && ap_escape_errorlog_item(scratch, referer, 8192 - len)
                              ...
                              const char *fmt, ...)
{
    ...
    log_error_core(file, line, level, status, s, ((void *)0), ((void *)0), ((void *)0), fmt, args);
                               ...
                               const char *fmt, ...)
{
    ...
    __builtin_va_start(args,fmt);
    log_error_core(file, line, level, status, ((void *)0), ((void *)0), ((void *)0), p, fmt, args);
    __builtin_va_end(args);
}
                               ...
                               const char *fmt, ...)
{
    ...
    __builtin_va_start(args,fmt);
    log_error_core(file, line, level, status, r->server, ((void *)0), r, ((void *)0), fmt,
                   args);
# 733 "log.c"
    __builtin_va_end(args);
    __builtin_va_start(args,fmt);
                               ...
                               const char *fmt, ...)
{
    ...
    __builtin_va_start(args,fmt);
    log_error_core(file, line, level, status, c->base_server, c, ((void *)0), ((void *)0),
                   fmt, args);
    __builtin_va_end(args);
}
...
void ap_log_pid(apr_pool_t *p, const char *filename)
{
    ...
    if (!fname) {
        ap_log_error("log.c",772, ((0x07 + 1) * 4)|2, (20000 + 24),
                     ((void *)0), "Invalid PID file path %s, ignoring.", filename);
        return;
    }
    ...
    if ((rv = apr_file_open(&pid_file, fname,
        ...
        != 0) {
        ap_log_error("log.c",797, 3, rv, ((void *)0),
                     "could not create %s", fname);
        ap_log_error("log.c",799, 3, 0, ((void *)0),
                                     ...
                                     pid_t *mypid)
{
    ...
    fname = ap_server_root_relative(p, filename);
    if (!fname) {
        ap_log_error("log.c",825, ((0x07 + 1) * 4)|2, (20000 + 24),
                     ((void *)0), "Invalid PID file path %s, ignoring.", filename);
        return (20000 + 14);
    }
                               ...
                               int nLine)
{
    ...
    ap_log_error("log.c",861, 2, 0, ((void *)0),
...
static apr_status_t piped_log_spawn(piped_log *pl)
{
    ...
    if (((status = apr_procattr_create(&procattr, pl->p)) != 0) ||
        ...
        ap_log_error("log.c",897, ((0x07 + 1) * 4), 0, ((void *)0),
        ...
        if ((status = apr_file_open_stdout(&outfile, pl->p)) == 0)
            status = apr_procattr_child_out_set(procattr, outfile, ((void *)0));
        if ((status = apr_file_open_stderr(&errfile, pl->p)) == 0)
            status = apr_procattr_child_err_set(procattr, errfile, ((void *)0));
        ...
        if (status == 0) {
            ...
            apr_proc_other_child_register(procnew, piped_log_maintenance, pl,
                                          ((pl)->fds[1]), pl->p);
            close_handle_in_child(pl->p, ((pl)->fds[0]));
        }
        else {
            char buf[120];

            ap_log_error("log.c",931, ((0x07 + 1) * 4), 0, ((void *)0),
...
static void piped_log_maintenance(int reason, void *data, int status)
{
    ...
    case 4:
        ...
        if (stats != 0) {
            ap_log_error("log.c",956, ((0x07 + 1) * 4), 0, ((void *)0),
                         "can't query MPM state; not restarting "
                         "piped log program '%s'",
                         pl->program);
        }
        else if (mpm_state != 2) {
            ap_log_error("log.c",962, ((0x07 + 1) * 4), 0, ((void *)0),
            ...
            if ((stats = piped_log_spawn(pl)) != 0) {
                ...
                char buf[120];
                ap_log_error("log.c",969, ((0x07 + 1) * 4), 0, ((void *)0),
...
piped_log * ap_open_piped_log(apr_pool_t *p, const char *program)
{
    ...
    pl->pid = ((void *)0);
    if (apr_file_pipe_create(&((pl)->fds[0]),
                             &((pl)->fds[1]), p) != 0) {
        return ((void *)0);
    }
