...
apr_status_t apr_password_validate(const char *passwd,
                                                const char *hash);
# 33 "htdbm.c" 2
# 1 "/usr/include/apr-1/apr_sha1.h" 1
# 51 "/usr/include/apr-1/apr_sha1.h"
...
# 82 "/usr/include/apr-1/apr_sha1.h"
...
static apr_status_t htdbm_init(apr_pool_t **pool, htdbm_t **hdbm)
{
    ...
    apr_signal(2, (void (*)(int)) htdbm_interrupted);

    (*hdbm) = (htdbm_t *)memset(apr_palloc(*pool, sizeof(htdbm_t)), 0, sizeof(htdbm_t));
    (*hdbm)->pool = *pool;
# 168 "htdbm.c"
    (*hdbm)->alg = 1;
    (*hdbm)->type = "default";
...
{
    if (htdbm->create)
        return apr_dbm_open_ex(&htdbm->dbm, htdbm->type, htdbm->filename, 3,
                            0x0FFF, htdbm->pool);
    else
...
static apr_status_t htdbm_verify(htdbm_t *htdbm)
{
    ...
    if (!apr_dbm_exists(htdbm->dbm, key))
        return 2;
    if (apr_dbm_fetch(htdbm->dbm, key, &val) != 0)
        return 2;
...
static apr_status_t htdbm_make(htdbm_t *htdbm)
{
    ...
    switch (htdbm->alg) {
        ...
        case 1:
            (void) srand((int) time((time_t *) ((void *)0)));
            to64(&salt[0], rand(), 8);
            salt[8] = '\0';
...
static void htdbm_usage(void)
{
    ...
    fprintf(stderr, "Usage: htdbm    [-cm""d""pstvx] [-TDBTYPE] database username\n");
    fprintf(stderr, "                -b[cm""d""ptsv] [-TDBTYPE] database username password\n");
    fprintf(stderr, "                -n[m""d""pst]   username\n");
    fprintf(stderr, "                -nb[m""d""pst]  username password\n");
    fprintf(stderr, "                -v[m""d""ps]    [-TDBTYPE] database username\n");
    fprintf(stderr, "                -vb[m""d""ps]   [-TDBTYPE] database username password\n");
    fprintf(stderr, "                -x[m""d""ps]    [-TDBTYPE] database username\n");
...
int main(int argc, const char * const argv[])
{
    ...
    if (need_pwd) {
        l = sizeof(pwc);
        if (apr_password_get("Enter password        : ", pwi, &l) != 0) {
            fprintf(stderr, "Password too long\n");
            exit(5);
        }
        l = sizeof(pwc);
        if (apr_password_get("Re-type password      : ", pwc, &l) != 0) {
            fprintf(stderr, "Password too long\n");
            exit(5);
        }
        if (strcmp(pwi, pwc) != 0) {
