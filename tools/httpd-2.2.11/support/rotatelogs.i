...
{
    char buf[65536], buf2[1024], errbuf[256];
    ...
    apr_file_t *f_stdin, *nLogFD = ((void *)0), *nLogFDprev = ((void *)0);
    apr_pool_t *pool;
    apr_pool_t *pfile = ((void *)0);
    apr_pool_t *pfile_prev = ((void *)0);
    apr_getopt_t *opt;
    apr_status_t rv;
    char c;
    const char *optarg;
    ...
    while ((rv = apr_getopt(opt, "lf", &c, &optarg)) == 0) {
        switch (c) {
        case 'l':
            use_localtime = 1;
            break;
    ...
    use_strftime = (strchr(szLogRoot, '%') != ((void *)0));
    if (apr_file_open_stdin(&f_stdin, pool) != 0) {
    ...
    for (;;) {
        ...
        if (!bypass_io) {
            if (apr_file_read(f_stdin, buf, &nRead) != 0) {
                exit(3);
            }
        }
        if (tRotation) {
            now = get_now(use_localtime, utc_offset);
            if (nLogFD != ((void *)0) && now >= tLogEnd) {
                nLogFDprev = nLogFD;
                nLogFD = ((void *)0);
            }
        }
            ...
            if ((nLogFD != ((void *)0)) &&
                (apr_file_info_get(&finfo, 0x00000100, nLogFD) == 0)) {
        ...
        if (nLogFD == ((void *)0)) {
            ...
            pfile_prev = pfile;
            apr_pool_create_ex(&pfile, pool, ((void *)0), ((void *)0));
            rv = apr_file_open(&nLogFD, buf2, 0x00002 | 0x00004 | 0x00008,
                               0x0FFF, pfile);
            if (rv != 0) {
                char error[120];
                ...
                if (nLogFDprev == ((void *)0)) {
                    ...
                    if (apr_file_write(nLogFD, errbuf, &nWrite) != 0) {
        ...
        if (!bypass_io) {
            ...
            if (nWrite != nRead) {
                ...
                cur_offset = 0;
                if (apr_file_seek(nLogFD, 1, &cur_offset) != 0) {
                    cur_offset = -1;
                }
