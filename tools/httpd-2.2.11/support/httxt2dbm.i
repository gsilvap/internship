...
# 31 "httxt2dbm.c" 2
# 1 "/usr/include/apr-1/apr_dbm.h" 1
# 20 "/usr/include/apr-1/apr_dbm.h"
# 1 "/usr/include/apr-1/apu.h" 1
# 21 "/usr/include/apr-1/apr_dbm.h" 2
# 42 "/usr/include/apr-1/apr_dbm.h"
...
static apr_status_t to_dbm(apr_dbm_t *dbm, apr_file_t *fp, apr_pool_t *pool)
{
    ...
    apr_pool_create_ex(&p, pool, ((void *)0), ((void *)0));
    ...
    while (apr_file_gets(line, sizeof(line), fp) == 0) {
        ...
        while (*c && !(((*__ctype_b_loc ())[(int) ((((unsigned char)(*c))))] & (unsigned short int) _ISspace))) {
            ++c;
        }
...
int main(int argc, const char *const argv[])
{
    ...
    apr_pool_create_ex(&pool, ((void *)0), ((void *)0), ((void *)0));
    ...
    if (rv != 0) {
        apr_file_printf(errfile, "Error: apr_getopt_init failed.""\n" "\n");
        return 1;
    }
    ...
    while ((rv = apr_getopt(opt, "vf::i::o::", &ch, &optarg)) == 0) {
        switch (ch) {
        case 'v':
            if (verbose) {
                apr_file_printf(errfile, "Error: -v can only be passed once" "\n" "\n");
                usage();
                return 1;
            }
            verbose = 1;
        ...
        case 'f':
            if (format) {
                apr_file_printf(errfile, "Error: -f can only be passed once" "\n" "\n");
                usage();
                return 1;
            }
        ...
        case 'i':
            if (input) {
                apr_file_printf(errfile, "Error: -i can only be passed once" "\n" "\n");
                usage();
                return 1;
            }
        ...
        case 'o':
            if (output) {
                apr_file_printf(errfile, "Error: -o can only be passed once" "\n" "\n");
                usage();
                return 1;
            }
    ...
    if (rv != ((20000 + 50000) + 14)) {
        apr_file_printf(errfile, "Error: Parsing Arguments Failed" "\n" "\n");
        usage();
        return 1;
    }

    if (!input) {
        apr_file_printf(errfile, "Error: No input file specified." "\n" "\n");
        usage();
        return 1;
    }

    if (!output) {
        apr_file_printf(errfile, "Error: No output DBM specified." "\n" "\n");
        usage();
        return 1;
    }
    ...
    if (verbose) {
        apr_file_printf(errfile, "DBM Format: %s""\n", format);
    }
    ...
    if (!strcmp(input, "-")) {
        ...
        rv = apr_file_open(&infile, input, 0x00001|0x00080,
                           0x0FFF, pool);
    }

    if (rv != 0) {
        apr_file_printf(errfile,
                        "Error: Cannot open input file '%s': (%d) %s" "\n" "\n",
                         input, rv, apr_strerror(rv, errbuf, sizeof(errbuf)));
        return 1;
    }

    if (verbose) {
        apr_file_printf(errfile, "Input File: %s""\n", input);
    }

    rv = apr_dbm_open_ex(&outdbm, format, output, 3,
                    0x0FFF, pool);
    ...
    if (rv != 0) {
        apr_file_printf(errfile,
                        "Error: Cannot open output DBM '%s': (%d) %s" "\n" "\n",
                         output, rv, apr_strerror(rv, errbuf, sizeof(errbuf)));
        return 1;
    }

    if (verbose) {
        apr_file_printf(errfile, "DBM File: %s""\n", output);
    }
    ...
    if (verbose) {
        apr_file_printf(errfile, "Conversion Complete." "\n");
    }
